...
void sparseTiraCol(SPARSE *A, long int n){
    ...
    while(atual != NULL)
    {
        if(atual->j > n){
            ...
            next = atual->prox;
            free(atual);
            if (anterior != NULL) anterior->prox = next;
            atual = next;
        }
        else{
            anterior = atual;
...
//    
    ...
    for(j=0;j<maxIT;j++){
        ...
        for (i=0;i<m;i++) r[i] = r[i] - alpha*w[i]; //r = r - alpha*w
        ...
        for (i=0;i<n;i++) p[i] = z[i] + beta*p[i]; //p = z + beta*p
...
//    while(atual != NULL)
//    {
//        fprintf(arquivo,"\n%d \t %d \t %.5lf",atual->i+1,atual->j+1,*atual->ij);
//        atual = atual->prox;
//    } 
//    fprintf(arquivo,"\nVetor b:");
//    for (i=0;i<m;i++){
...
//Solução de sistema linear esparso via gradientes biconjugados
//recebe ponto inicial x0
//http://utminers.utep.edu/xzeng/2017spring_math5330/MATH_5330_Computational_Methods_of_Linear_Algebra_files/ln07.pdf
//https://www-users.cs.umn.edu/~saad/IterMethBook_2ndEd.pdf
/*void sparseBCGS(SPARSE *A, double *b, double *x, long int n) {
    int i, j;
    double r[n],rs[n],aux[n],aux2[n],p[n],s[n],Dx[n];
    double alpha, beta, omega, tol = 0.00001;
    ...
    for(j=0;j<maxIT;j++){
        ...
        sparseMultMatVet(A, p, aux);
        double alpha1 = prod_escalar(r,rs,n);
        alpha = alpha1/prod_escalar(aux,rs,n);
        for (i=0;i<n;i++){
            s[i] = r[i] - alpha*aux[i];
        }
        for (i=0;i<n;i++) aux2[i] = 0;
        sparseMultMatVet(A, s, aux2);
        omega = prod_escalar(aux2,s,n)/prod_escalar(aux2,aux2,n);
        
        for (i=0;i<n;i++){
            Dx[i] = alpha*p[i] + omega*s[i];
            x[i] += Dx[i];
            r[i] = s[i] - omega*aux2[i];
        }
        if (norma_inf(r,n) < tol) break;
        
        beta = alpha/omega*prod_escalar(r,rs,n)/alpha1;
        for(i=0;i<n;i++){
            p[i] = r[i] + beta*(p[i] + omega*aux[i]);
        }
        printf("\nIT: %d",j);
        //Convergência com tolerância numérica
        if (norma_inf(Dx,n) < tol) break;       

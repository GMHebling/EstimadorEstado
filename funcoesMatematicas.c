...
// Soluciona Lx = b e salva a resposta no vetor b (L no formato compressed
// column)
...
void sparseCG_preCholesky(CCSPARSE *L, SPARSE *A, double *b, double *x,
                          long int n) {
  int i, j;
...
// Gradiente conjugado para equaçao normal com precondicionador a esquerda
// (trinagular CCSPARSE)
void sparsePCGLS(CCSPARSE *L, SPARSE *A, double *b, double *x, long int m,
                 long int n) {
  int i, j;
...
void sparseCGLS(SPARSE *A, SPARSE *At, double *b, double *x, long int m,
                long int n) {
  int i, j;
  double r[n], p[n], Dx[n];
  double d[m], t[m];
...
// Gradiente conjugado para equaçao normal - précondicionado - Jacobi
// preconditioner
void sparseCGLS_preJacobi(double *M, SPARSE *A, SPARSE *At, double *b,
                          double *x, long int m, long int n) {
  int i, j;
...
// Gradiente conjugado para equaçao normal - précondicionado - Cholesky
// preconditioner
void sparseCGLS_preCholesky(CCSPARSE *L, SPARSE *A, SPARSE *At, double *b,
                            double *x, long int m, long int n) {
  int i, j;
  double Mr[n], r[n], p[n], Dx[n];
  double d[m], t[m];
...
__complex__ double **c_matAloca(int n) {
  int i, j;
  __complex__ double **m =
      (__complex__ double **)malloc(n * sizeof(__complex__ double *));
...
// Inversa da matriz método Decomposição LU (Para calcular a matriz admitância
// de linhas)
...
// Iguala Matrizes B = A
long int mat_ig_sparse(double ***A, int m, int n, long int *i_sparse,
                       long int *j_sparse, double *x_sparse) {
  int i, j;
  int index = 0;
  for (i = 0; i < m; i++) {
    for (j = 0; j < n; j++) {
...
long int tira_refs_sparse(DMED *medidas, double ***A, int m, int n, int col1,
                          int col2, long int *sparse_i, long int *sparse_j,
                          double *sparse_x, double *regua, double *x,
                          long int it) {
  int i, j, n_cols;
  int index_i = 0;
  int index_j = 0;
...
void tira_refs(double ***A, int m, int n, int col1, int col2, double **temp,
               double *regua, double *x, long int it) {
  int i, j, n_cols;
  int index = 0;
...
void cat_hor(double **A, int m1, int n1, double **B, int m2, int n2,
             double **temp) {
  int i, j;
...
void mult_mat(double **A, int m1, int n1, double **B, int m2, int n2,
              double **C) {
  int i, j, k;
  double aux_sum;
...
// Concatena horizontalmente duas matrizes
void cat_vert(double **A, int m1, int n1, double **B, int m2, int n2,
              double **temp) {
  int i, j;
...
// Efetua a backward substituion em matriz triangular superior e retorna o vetor
// x no lugar de b
...
// Efetua a forward substituion em matriz triangular inferior e retorna o vetor
// x no lugar de b

...
void Sk(GRAFO *grafo, long int k, __complex__ double *S) {
  ...
  for (j = 0; j < 3; j++) {
    for (i = 0; i < 3; i++) {
      S[j] =
          S[j] + grafo[k].V[j] * conj(grafo[k].Ysh[j][i]) * conj(grafo[k].V[i]);
    }
  }
...
//------------------------------------------------------------------------------
//
// FUNÇÕES DE CÁLCULO ELETRICO - DERIVADAS DOS FLUXOS DE POTÊNCIA, CORRENTE
// ETC..
//
//------------------------------------------------------------------------------
...
void dSkm(GRAFO *noP, GRAFO *noS, DRAM *ramo, __complex__ double *dS,
          long int opt, long int i) { // fase i
  int j, k;
  ...
  case 0: // Derivada em relação a tensão do noP - DSp / DVpi
    for (k = 0; k < 3; k++) {
      dS[k] = 0;
      for (j = 0; j < 3; j++) {
        dS[k] += J[k][k] * 1 / cabs(Vp[k]) * Vp[k] * conj(ramo->Ypp[k][j]) *
                     conj(Vp[j]) +
                 J[k][k] * 1 / cabs(Vp[k]) * Vp[k] * conj(ramo->Yps[k][j]) *
                     conj(Vs[j]);
      }
  ...
  case 1: // Derivada em relação ao ângulo do noP - DSp / DVpi
    for (k = 0; k < 3; k++) {
      dS[k] = 0;
      for (j = 0; j < 3; j++) {
        dS[k] += I * Vp[k] * J[k][k] * conj(ramo->Ypp[k][j]) * conj(Vp[j]) +
                 I * Vp[k] * J[k][k] * conj(ramo->Yps[k][j]) * conj(Vs[j]);
      }
...
// void dSmk
void dSmk(GRAFO *noP, GRAFO *noS, DRAM *ramo, __complex__ double *dS,
          long int opt, long int i) { // fase i
  int j, k;
  ...
  case 2: // Derivada em relação a tensão do noS - DSp / DVpi
    for (k = 0; k < 3; k++) {
      dS[k] = 0;
      for (j = 0; j < 3; j++) {
        dS[k] += J[k][k] * 1 / cabs(Vs[k]) * Vs[k] * conj(ramo->Ysp[k][j]) *
                     conj(Vp[j]) +
                 J[k][k] * 1 / cabs(Vs[k]) * Vs[k] * conj(ramo->Yss[k][j]) *
                     conj(Vs[j]);
      }
  ...
  case 3: // Derivada em relação ao ângulo do noS - DSp / DVpi
    for (k = 0; k < 3; k++) {
      dS[k] = 0;
      for (j = 0; j < 3; j++) {
        dS[k] += I * Vs[k] * J[k][k] * conj(ramo->Ysp[k][j]) * conj(Vp[j]) +
                 I * Vs[k] * J[k][k] * conj(ramo->Yss[k][j]) * conj(Vs[j]);
      }
...
void dSk(GRAFO *grafo, long int k, __complex__ double *dS, long int opt,
  ...
  if (barra == k) {
    ...
    if (opt == 0) { // Derivada em relação a tensão do noP - DSp / DVpi
      for (j = 0; j < 3; j++) {
        for (i = 0; i < 3; i++) {
          dS[j] += J[j][j] * 1 / cabs(grafo[k].V[j]) * grafo[k].V[j] *
                   conj(grafo[k].Ysh[j][i]) * conj(grafo[k].V[i]);
        }
        dS[j] += grafo[k].V[j] * conj(grafo[k].Ysh[j][fase]) * 1 /
                 cabs(grafo[k].V[fase]) * conj(grafo[k].V[fase]);
      }
    } else { // opt==1 >> Derivada em relação ao angulo do noP - DSp / DVpi
      for (j = 0; j < 3; j++) {
        for (i = 0; i < 3; i++) {
          dS[j] += I * grafo[k].V[j] * J[j][j] * conj(grafo[k].Ysh[j][i]) *
                   conj(grafo[k].V[i]);
        }
        dS[j] += grafo[k].V[j] * conj(grafo[k].Ysh[j][fase]) * (-1) * I *
                 conj(grafo[k].V[fase]);
      }
    }
  } else {
...
//
//------------------------------------------------------------------------------
void dSkm_ret(GRAFO *noP, GRAFO *noS, DRAM *ramo, __complex__ double *dS,
              long int opt, long int i) { // fase i
  int j, k;
  ...
  case 0: // Derivada em relação a tensão real do noP - DSp / DVpi
    for (k = 0; k < 3; k++) {
      dS[k] = 0;
      for (j = 0; j < 3; j++) {
        dS[k] += conj(ramo->Ypp[k][j]) * __real__ Vp[j] -
                 I * conj(ramo->Ypp[k][j]) * __imag__ Vp[j];
        dS[k] += conj(ramo->Yps[k][j]) * __real__ Vs[j] -
                 I * conj(ramo->Yps[k][j]) * __imag__ Vs[j];
      }
      if (k != i)
        dS[k] = 0; // Jff x
      dS[k] += __real__ Vp[k] * conj(ramo->Ypp[k][i]) -
               I * __imag__ Vp[k] * conj(ramo->Ypp[k][i]);
    }
    break;
  case 1: // Derivada em relação a tensão imaginária do noP - DSp / DVpi
    for (k = 0; k < 3; k++) {
      dS[k] = 0;
      for (j = 0; j < 3; j++) {
        dS[k] += I * conj(ramo->Ypp[k][j]) * __real__ Vp[j] +
                 conj(ramo->Ypp[k][j]) * __imag__ Vp[j];
        dS[k] += I * conj(ramo->Yps[k][j]) * __real__ Vs[j] +
                 conj(ramo->Yps[k][j]) * __imag__ Vs[j];
      }
      if (k != i)
        dS[k] = 0; // Jff x
      dS[k] += -I * __real__ Vp[k] * conj(ramo->Ypp[k][i]) +
               __imag__ Vp[k] * conj(ramo->Ypp[k][i]);
    }
    break;
  case 2: // Derivada em relação a tensão real do noS - DSp / DVpi
    for (k = 0; k < 3; k++) {
      dS[k] = __real__ Vp[k] * conj(ramo->Yps[k][i]) +
              I * __imag__ Vp[k] * conj(ramo->Yps[k][i]);
    }
    break;
  case 3: // Derivada em relação a tensão imaginária do noS - DSp / DVpi
    for (k = 0; k < 3; k++) {
      dS[k] = -I * __real__ Vp[k] * conj(ramo->Yps[k][i]) +
              __imag__ Vp[k] * conj(ramo->Yps[k][i]);
    }
    break;
  }
  free(J);
...
// void dSmk
void dSmk_ret(GRAFO *noP, GRAFO *noS, DRAM *ramo, __complex__ double *dS,
              long int opt, long int i) { // fase i
  int j, k;
  __complex__ double Vp[3], Vs[3], **J = NULL;
  ...
  case 0: // Derivada em relação a tensão real do noP - DSs / DVpi
    for (k = 0; k < 3; k++) {
      dS[k] = __real__ Vs[k] * conj(ramo->Ysp[k][i]) +
              I * __imag__ Vp[k] * conj(ramo->Ysp[k][i]);
    }
    break;
  case 1: // Derivada em relação a tensão imaginária do noP - DSs / DVpi
    for (k = 0; k < 3; k++) {
      dS[k] = -I * __real__ Vs[k] * conj(ramo->Ysp[k][i]) +
              __imag__ Vp[k] * conj(ramo->Ysp[k][i]);
    }
    break;
  case 2: // Derivada em relação a tensão real do noS - DSs / DVpi
    for (k = 0; k < 3; k++) {
      dS[k] = 0;
      for (j = 0; j < 3; j++) {
        dS[k] += conj(ramo->Ysp[k][j]) * __real__ Vp[j] -
                 I * conj(ramo->Ysp[k][j]) * __imag__ Vp[j];
        dS[k] += conj(ramo->Yss[k][j]) * __real__ Vs[j] -
                 I * conj(ramo->Yss[k][j]) * __imag__ Vs[j];
      }
      if (k != i)
        dS[k] = 0; // Jff x
      dS[k] += __real__ Vs[k] * conj(ramo->Yss[k][i]) +
               I * __imag__ Vs[k] * conj(ramo->Yss[k][i]);
    }
    break;
  case 3: // Derivada em relação ao ângulo do noS - DSs / DVpi
    for (k = 0; k < 3; k++) {
      dS[k] = 0;
      for (j = 0; j < 3; j++) {
        dS[k] += I * conj(ramo->Ysp[k][j]) * __real__ Vp[j] +
                 conj(ramo->Ysp[k][j]) * __imag__ Vp[j];
        dS[k] += I * conj(ramo->Yss[k][j]) * __real__ Vs[j] +
                 conj(ramo->Yss[k][j]) * __imag__ Vs[j];
      }
      if (k != i)
        dS[k] = 0; // Jff x
      dS[k] += -I * __real__ Vs[k] * conj(ramo->Yss[k][i]) +
               __imag__ Vs[k] * conj(ramo->Yss[k][i]);
    }
    break;
  }
  free(J);
...
// opt = 0 tensão
// opt = 1 angulo em relação à fase "i" da barra
void dSk_ret(GRAFO *grafo, long int k, __complex__ double *dS, long int opt,
             long int barra, long int fase) {
  int i, j, opt1;
  ...
  if (barra == k) {
    for (i = 0; i < grafo[k].numeroAdjacentes; i++) {
      if ((k == grafo[k].adjacentes[i].ramo->k)) {
        if (opt == 0)
          ...
          opt1 = 1;
        dSkm_ret(&grafo[k], &grafo[grafo[k].adjacentes[i].idNo],
                 grafo[k].adjacentes[i].ramo, dSaux, opt1, fase);
      } else {
        if (opt == 0)
          opt1 = 2;
          ...
          opt1 = 3;
        dSmk_ret(&grafo[grafo[k].adjacentes[i].idNo], &grafo[k],
                 grafo[k].adjacentes[i].ramo, dSaux, opt1, fase);
      }
      for (j = 0; j < 3; j++) {
        dS[j] = dS[j] - dSaux[j];
    ...
    if (opt == 0) { // Derivada em relação a tensão real do noP - DSp / DVpi
      for (j = 0; j < 3; j++) {
        if (j == fase) {
          for (i = 0; i < 3; i++) {
            dS[j] += conj(grafo[k].Ysh[j][i]) * __real__ grafo[k].V[i] -
                     I * conj(grafo[k].Ysh[j][i]) * __imag__ grafo[k].V[i];
          }
        } // if (j!=fase) dS[j] = 0; //Jff x
        dS[j] += __real__ grafo[k].V[j] * conj(grafo[k].Ysh[j][fase]) -
                 I * __imag__ grafo[k].V[j] * conj(grafo[k].Ysh[j][fase]);
      }
    } else { // opt==1 >> Derivada em relação a tensao imaginaria do noP - DSp /
             // DVpi
      for (j = 0; j < 3; j++) {
        if (j == fase) {
          for (i = 0; i < 3; i++) {
            dS[j] += I * conj(grafo[k].Ysh[j][i]) * __real__ grafo[k].V[i] +
                     conj(grafo[k].Ysh[j][i]) * __imag__ grafo[k].V[i];
          }
        } //  if (j!=fase) dS[j] = 0; //Jff x
        dS[j] += -I * __real__ grafo[k].V[j] * conj(grafo[k].Ysh[j][fase]) +
                 __imag__ grafo[k].V[j] * conj(grafo[k].Ysh[j][fase]);
      }
    }
...
// opt = 2 tensão noS
// opt = 3 angulo noS em relação à fase "i"
void dIkm(GRAFO *noP, GRAFO *noS, DRAM *ramo, __complex__ double *dI,
          long int opt, long int i) { // fase i
  int j, k;
  ...
  case 0: // Derivada em relação a tensão do noP - DSp / DVpi
    for (k = 0; k < 3; k++) {
      //                dI = 1/(bar[k].V)*(Pkm(km)/S*dPkm(km,3) +
      //                Qkm(km)/S*dQkm(km,3)) - S/pow(bar[k].V,2);
      dI[k] = 1 / cabs(Vp[i]) *
                  (__real__ S[k] / cabs(S[k]) * __real__ dS[k] +
                   __imag__ S[k] / cabs(S[k]) * __imag__ dS[k]) -
              S[k] / pow(cabs(Vp[i]), 2);
    }
    break;
  case 1: // Derivada em relação ao ângulo do noP - DSp / DVpi
    for (k = 0; k < 3; k++) {
      //                dI = 1/(bar[k].V)*(Pkm(km)/S*dPkm(km,1) +
      //                Qkm(km)/S*dQkm(km,1));
      dI[k] = 1 / cabs(Vp[i]) *
              (__real__ S[k] / cabs(S[k]) * __real__ dS[k] +
               __imag__ S[k] / cabs(S[k]) * __imag__ dS[k]);
  ...
  case 2: // Derivada em relação a tensão do noS - DSp / DVpi
    for (k = 0; k < 3; k++) {
      //                dI = 1/(bar[k].V)*(Pkm(km)/S*dPkm(km,2) +
      //                Qkm(km)/S*dQkm(km,2));
      dI[k] = 1 / cabs(Vp[i]) *
              (__real__ S[k] / cabs(S[k]) * __real__ dS[k] +
               __imag__ S[k] / cabs(S[k]) * __imag__ dS[k]);
  ...
  case 3: // Derivada em relação ao ângulo do noS - DSp / DVpi
    for (k = 0; k < 3; k++) {
      //                dI = 1/(bar[k].V)*(Pkm(km)/S*dPkm(km,4) +
      //                Qkm(km)/S*dQkm(km,4));
      dI[k] = 1 / cabs(Vp[i]) *
              (__real__ S[k] / cabs(S[k]) * __real__ dS[k] +
               __imag__ S[k] / cabs(S[k]) * __imag__ dS[k]);
...
// opt = 2 tensão noS
// opt = 3 angulo noS em relação à fase "i"
void dImk(GRAFO *noP, GRAFO *noS, DRAM *ramo, __complex__ double *dI,
          long int opt, long int i) { // fase i
  int j, k;
  __complex__ double Vp[3], Vs[3], **J = NULL, P1, P2, P3;
  __complex__ double dS[3];

...
DMED_COMPLEX *converte_medidas_para_complexo(DMED *medidas,
...
DMED_COMPLEX *divide_medidas_por_tensao(DMED_COMPLEX *medidas_complexas,
                                        long int numeroMedidas,
                                        long int numeroBarras, GRAFO *grafo) {

  DMED_COMPLEX *medidas_div = NULL;
  if (((medidas_div) = (DMED_COMPLEX *)malloc((numeroMedidas) *
                                              sizeof(DMED_COMPLEX))) == NULL) {
    printf("Erro -- Nao foi possivel alocar espaco de memoria para as medidas "
           "complexas !!!!");
    exit(1);
  }
  ...
  for (int cont = 0; cont < numeroMedidas; cont++) {
    ...
    // printf("%f + i*%f\n", creal(medidas_div[cont].zmed),
    // cimag(medidas_div[cont].zmed));
    ...
    // printf("%f + i*%f\n", creal(medidas_div[cont].zmed),
    // cimag(medidas_div[cont].zmed)); printf("\n");
...
void monta_regua_x(long int numeroRamos, double *regua_x, DRAM *ramos) {
  for (int nr = 0; nr < numeroRamos; nr++) {
    ...
    // printf("de: %ld, para: %ld, regua: %.4f\n", ramos[nr].DE, ramos[nr].PARA,
    // regua_x[3*nr]);
  }
}

void monta_regua_medidas(long int nmed_BC, double *regua_med,
                         double *regua_med_inv,
                         DMED_COMPLEX *medidas_equivalentes) {
  for (int nm = 0; nm < nmed_BC; nm++) {
    if (medidas_equivalentes[nm].tipo == 0 |
        medidas_equivalentes[nm].tipo == 1) {
      switch (medidas_equivalentes[nm].fases) {
      case 1:
        regua_med[3 * nm] = medidas_equivalentes[nm].DE +
                            medidas_equivalentes[nm].PARA / 10000.0;
        regua_med_inv[3 * nm] = -1 * (medidas_equivalentes[nm].PARA +
                                      medidas_equivalentes[nm].DE / 10000.0);
        break;
      case 2:
        regua_med[3 * nm + 1] = 2.0 * (medidas_equivalentes[nm].DE +
                                       medidas_equivalentes[nm].PARA / 10000.0);
        regua_med_inv[3 * nm + 1] =
            2.0 * (-1 * (medidas_equivalentes[nm].PARA +
                         medidas_equivalentes[nm].DE / 10000.0));
        break;
      case 3:
        regua_med[3 * nm + 2] = 3.0 * (medidas_equivalentes[nm].DE +
                                       medidas_equivalentes[nm].PARA / 10000.0);
        regua_med_inv[3 * nm + 2] =
            3.0 * (-1 * (medidas_equivalentes[nm].PARA +
                         medidas_equivalentes[nm].DE / 10000.0));
        break;
      case 4:
        regua_med[3 * nm] = medidas_equivalentes[nm].DE +
        ...
        regua_med[3 * nm + 1] = 2 * (medidas_equivalentes[nm].DE +
                                     medidas_equivalentes[nm].PARA / 10000.0);
        regua_med_inv[3 * nm + 1] =
            2 * (-1 * (medidas_equivalentes[nm].PARA +
                       medidas_equivalentes[nm].DE / 10000.0));
      ...
      case 5:
        ...
        regua_med[3 * nm + 2] = 3 * (medidas_equivalentes[nm].DE +
                                     medidas_equivalentes[nm].PARA / 10000.0);
        regua_med_inv[3 * nm + 2] =
            3 * (-1 * (medidas_equivalentes[nm].PARA +
                       medidas_equivalentes[nm].DE / 10000.0));
        break;
      case 6:
        regua_med[3 * nm + 1] = 2 * (medidas_equivalentes[nm].DE +
...
double **monta_matriz_H(long int numeroRamos, long int nmed_BC, double *regua_x,
                        double *regua_med, double *regua_med_inv) {
  double **H_BC = NULL;
  int ctnz = 0;
  ...
  for (int nm = 0; nm < 3 * nmed_BC; nm++) {
    for (int nv = 0; nv < 3 * numeroRamos; nv++) {
      ...
      if (regua_med[nm] != 0.0 && regua_x[nv] != 0.0) {
        ...
        if (regua_x[nv] - regua_med[nm] > 0 &&
            regua_x[nv] - regua_med[nm] < 1.0) {

          H_BC[nm][nv] = 1.0;
          // printf("x: %f ---- med: %f ---- H: %f\n", regua_x[nv],
          // regua_med[nm], H_BC[nm][nv]);
        }
        if (fabs(((regua_x[nv] - (int)regua_x[nv]) * 10000.0) - regua_med[nm]) <
            EPS) {
...
int verifica_ramo_caminho(double *regua_caminho, double *regua_x, int idx_ramo,
                          long int numeroRamos, int limite_caminho) {
...
void monta_regua_caminho(long int numeroRamos, long int numeroBarras,
                         double *regua_caminho, int *caminho, GRAFO *grafo) {
...
double calculo_theta_dv(GRAFO *grafo, int i_grafo, int i_fase,
                        double *valor_hx_v) {
...
// TODO: trocar o tipo da funcao para void e adicionar as matrizes H_real e
// H_imag nos parametros
double **monta_matriz_H_tensao(long int numeroBarras, long int numeroRamos,
                               int nmed_T, int *caminho,
                               DMED_COMPLEX *medidas_tensao, double *regua_x,
                               double *regua_caminho, DRAM *ramos, GRAFO *grafo,
                               double *hx_V) {
  ...
  for (i = 0; i < nmed_T; i++) {
    for (j = 0; j < numeroRamos; j++) {
      ...
      if (verifica_ramo_caminho(regua_caminho, regua_x, j, numeroRamos,
                                limite_caminho) == 1) {
...
double *resolve_linear_QR(double **H_BC, double *z, long int numeroRamos,
                          long int nmed_BC) {
  ...
  c = &Common;
  cholmod_l_start(c);
  T = cholmod_l_allocate_triplet(3 * nmed_BC, 3 * numeroRamos,
                                 3 * nmed_BC * 3 * numeroRamos, 0,
                                 CHOLMOD_COMPLEX, c);
  A = cholmod_l_allocate_sparse(3 * nmed_BC, 3 * numeroRamos,
                                3 * nmed_BC * 3 * numeroRamos, 0, 0, 0,
                                CHOLMOD_COMPLEX, c);
  b = cholmod_l_allocate_dense(3 * nmed_BC, 1, 3 * nmed_BC, CHOLMOD_COMPLEX, c);
  X = cholmod_l_allocate_dense(3 * numeroRamos, 1, 3 * numeroRamos,
                               CHOLMOD_COMPLEX, c);
  int index = 0;
...
double *resolve_linear_QR_Tensao(double **H_BC, double **H_T, double *z,
                                 long int numeroRamos, long int nmed_BC,
  ...
  c = &Common;
  cholmod_l_start(c);
  T = cholmod_l_allocate_triplet(6 * nmed_BC + nmed_T, 6 * numeroRamos,
                                 (6 * nmed_BC + nmed_T) * 3 * numeroRamos, 0,
                                 CHOLMOD_REAL, c);

  // G = cholmod_l_allocate_sparse(6 * numeroRamos,6 * numeroRamos, (6 *
  // numeroRamos* 6*numeroRamos), 0, 0, 1, CHOLMOD_REAL, c);
  A = cholmod_l_allocate_sparse(6 * nmed_BC + nmed_T, 6 * numeroRamos,
                                (6 * nmed_BC + nmed_T) * 3 * numeroRamos, 0, 0,
                                0, CHOLMOD_REAL, c);
  // AT = cholmod_l_allocate_sparse( 6 * numeroRamos,6 * nmed_BC + nmed_T, (6 *
  // nmed_BC + nmed_T) * 3 * numeroRamos, 0, 0, 0, CHOLMOD_REAL, c);
  b = cholmod_l_allocate_dense(6 * nmed_BC + nmed_T, 1, 6 * nmed_BC + nmed_T,
                               CHOLMOD_REAL, c);
  bH = cholmod_l_allocate_dense(6 * numeroRamos, 1, 6 * numeroRamos,
                                CHOLMOD_REAL, c);
  X = cholmod_l_allocate_dense(6 * numeroRamos, 1, 6 * numeroRamos,
                               CHOLMOD_REAL, c);
  int index = 0;
  for (int i = 0; i < 3 * nmed_BC; i++) {
    for (int r = 0; r < 3 * numeroRamos; r++) {
  ...
  A = cholmod_l_triplet_to_sparse(T, (6 * nmed_BC + nmed_T) * 3 * numeroRamos,
                                  c);
...
void monta_z_complexa(DMED_COMPLEX *medidas_eq, __complex__ double *z,
                      long int nmed_BC) {
...
void monta_z_real_e_imag(DMED_COMPLEX *medidas_eq, double *z, long int nmed_BC,
                         DMED_COMPLEX *medidas_tensao, int nmed_T) {
  for (int i = 0; i < nmed_BC; i++) {
    switch (medidas_eq[i].fases) {
    case 1:
...
void incializa_tensoes_grafo(GRAFO *grafo, long int numeroBarras,
...
void atualiza_Rede_BC(GRAFO *grafo, long int numeroBarras, DBAR *barra,
                      double *regua_x, long int numeroRamos, double *x_bc) {
  ...
  for (i = 0; i < numeroBarras; i++) {
    ...
    for (k = 0; k < (grafo)[i].numeroAdjacentes; k++) {
      aux_regua = barra[(grafo)[i].idNo].ID +
                  barra[(grafo)[i].adjacentes[k].idNo].ID / 10000.0;
      aux_regua_inv = -(barra[(grafo)[i].adjacentes[k].idNo].ID +
                        barra[(grafo)[i].idNo].ID / 10000.0);
      ...
      for (j = 0; j < numeroRamos; j++) {
        ...
        if (fabs(aux_regua - regua_x[3 * j]) < EPS) {
          ...
          case 1:
            ...
            break;
          case 2:
            (grafo)[i].adjacentes[k].Cur[1] =
                x_bc[6 * j + 2] + I * x_bc[6 * j + 3];
            break;
          case 3:
            (grafo)[i].adjacentes[k].Cur[2] =
                x_bc[6 * j + 4] + I * x_bc[6 * j + 5];
            break;
          case 4:
            (grafo)[i].adjacentes[k].Cur[0] = x_bc[6 * j] + I * x_bc[6 * j + 1];
            (grafo)[i].adjacentes[k].Cur[1] =
        ...
        if (fabs(regua_x[3 * j] + aux_regua_inv) < EPS) {
          ...
          switch (grafo[i].adjacentes[k].ramo->fases) {
          case 1:
            (grafo)[i].adjacentes[k].Cur[0] =
                -(x_bc[6 * j] + I * x_bc[6 * j + 1]);
            break;
          case 2:
            (grafo)[i].adjacentes[k].Cur[1] =
                -(x_bc[6 * j + 2] + I * x_bc[6 * j + 3]);
  ...
  // printf("\ni: %d -> k: %d = %f + j*%f", 0, 0,
  // creal(grafo[0].adjacentes[0].Cur[0]),
  // cimag(grafo[0].adjacentes[0].Cur[0]));
}

void atualiza_Rede_BC_Tensao(GRAFO *grafo, long int numeroBarras, DBAR *barra,
                             double *regua_x, long int numeroRamos,
                             double *x_bc) {
  int i, j, k, idMed, de, para, ramo, fase;
  __complex__ double *Saux, *Iaux;
  double aux_regua;
    ...
    for (k = 0; k < (grafo)[i].numeroAdjacentes; k++) {
      ...
      for (j = 0; j < numeroRamos; j++) {
        ...
        if (fabs(aux_regua - regua_x[3 * j]) < EPS) {
          ...
          case 1:
            (grafo)[i].adjacentes[k].Cur[0] =
                x_bc[3 * j] + I * x_bc[3 * j + numeroRamos];
            break;
          case 2:
            (grafo)[i].adjacentes[k].Cur[1] =
                x_bc[3 * j + 1] + I * x_bc[3 * j + 1 + numeroRamos];
            break;
          case 3:
            (grafo)[i].adjacentes[k].Cur[2] =
                x_bc[3 * j + 2] + I * x_bc[3 * j + 2 + numeroRamos];
            break;
          case 4:
            (grafo)[i].adjacentes[k].Cur[0] =
                x_bc[3 * j] + I * x_bc[3 * j + numeroRamos];
          ...
          case 5:
            (grafo)[i].adjacentes[k].Cur[0] =
                x_bc[3 * j] + I * x_bc[3 * j + 1 + numeroRamos];
            (grafo)[i].adjacentes[k].Cur[2] =
                x_bc[3 * j + 2] + I * x_bc[3 * j + 2 + numeroRamos];
            break;
          case 6:
...
void estimadorBC_RECT(GRAFO *grafo, long int numeroRamos, long int numeroBarras,
  ...
  medidas_equivalentes =
      (DMED_COMPLEX *)malloc((nmed_BC) * sizeof(DMED_COMPLEX));

  __complex__ double *z_eq = NULL;
  z_eq = c_vetAloca(3 * nmed_BC);
  // z_eq = (__complex__ double *)malloc(3*nmed_BC * sizeof(__complex__
  // double));
  ...
  printf("2\n");
  medidas_equivalentes = divide_medidas_por_tensao(medidas_complexas, nmed_BC,
                                                   numeroBarras, grafo);
  ...
  printf("5\n");
  H_BC =
      monta_matriz_H(numeroRamos, nmed_BC, regua_x, regua_med, regua_med_inv);
  printf("6\n");
  int it = 0;
  int conv = 0;
  while (conv < 1) {
    ...
    // H_BC = monta_matriz_H(numeroRamos, nmed_BC, regua_x, regua_med,
    // regua_med_inv);
    int st = 0;
    ...
    //         printf("\ni: %d -> k: %d = %f + j*%f", nb, nj,
    //         creal(grafo[nb].adjacentes[nj].Cur[0]),
    //         cimag(grafo[nb].adjacentes[nj].Cur[0]));
    //
    //     }
    // }
    ...
    if (nfx<tol | it> 30) {
      ...
      //             if (fabs(regua_med[nm] - regua_x[nv]) < EPS)
      //             {
      //                 __complex__ double residuo = x_bc[2 * nv] + I * x_bc[2
      //                 * nv + 1] - z_eq[nm];
...
void buscaProfundidadeLoop(GRAFO *grafo, int idNo, int barraAnterior,
                           BOOL *visitado, int *caminho, int *contBarras,
                           int *barraEntrada, int *nadj_proxbarra) {
  int i;
  int proxBarra;
  ...
  for (i = 0; i < grafo[idNo].numeroAdjacentes; i++) {
    ...
    if ((proxBarra != barraEntrada[idNo])) {
      if (visitado[proxBarra] == true) {
        ...
        // grafo[proxBarra].barra->ID); primeiro adjacente pode ser diferente da
        // barra imediatamente antes contar os loops e salvar na estrutura
        continue;
      } else {
        barraAnterior = idNo;
        contBarras[0] += 1;
        ...
        buscaProfundidadeLoop(grafo, proxBarra, barraAnterior, visitado,
                              caminho, contBarras, barraEntrada,
                              nadj_proxbarra);
      }
    } else {
      continue;
...
void busca_loop_grafo(GRAFO *grafo, long int numeroRamos, long int numeroBarras,
                      int *caminho, int *nadj_proxbarra) {
  // funcao recursiva
  ...
  buscaProfundidadeLoop(grafo, noRaiz, barraAnterior, visitado, caminho,
                        contBarras, barraEntrada, nadj_proxbarra);
  // printf("N_loops: %d\n", n_loops[0]);
  printf("\n");
  // for (i = 0; i < numeroBarras; i++){
...
DMED_COMPLEX *calcula_medida_tensao_complexa(DMED *medidas,
                                             long int numeroMedidas,
                                             GRAFO *grafo, int numeroBarras) {
  ...
  for (int contador = 0; contador < numeroMedidas; contador++) {
    if (medidas[contador].tipo == 4) {
      ...
      int idx_barra_DE =
          calcula_idx_ID(grafo, medida_tensao[aux_contador].DE, numeroBarras);
...
void monta_regua_medidas_tensao(DMED_COMPLEX *medidas_tensao, int nmed_T,
                                double *regua_V) {
  // regua = DE + 0.FASE
...
void calcula_hx_corrente(double **H_BC, double *x, double *hx, int nmed_BC,
                         int numeroRamos) {
  int i, j, k;
  double aux_r = 0;
  double aux_x = 0;
...
void estimadorBC_RECT_Malhado(GRAFO *grafo, long int numeroRamos,
                              long int numeroBarras, DMED *medidas,
                              long int **numeroMedidas,
                              ALIMENTADOR *alimentadores,
                              long int numeroAlimentadores, DRAM *ramos,
                              double Sbase, DBAR *barra) {
  long int nmed, nvar;
  int i, j, k, r;
  double *z = NULL, **h = NULL, ***H = NULL, **W = NULL, *x = NULL,
         *regua = NULL, aux = 0;
  ...
  printf("Estimador de Estado Branch Current em Coordenadas retangulares com "
         "medidas de tensão...\n");
  //--------------------------------------------------------------------------
  // Alocação de memória das variáveis do estimador de estado
  ...
  //
  medidas_tensao =
      calcula_medida_tensao_complexa(medidas, nmed, grafo, numeroBarras);
  ...
  H_T = monta_matriz_H_tensao(numeroBarras, numeroRamos, nmed_T, caminho,
                              medidas_tensao, regua_x, regua_caminho, ramos,
                              grafo, hx_V);
  ...
  while (conv < 1) {
    ...
    monta_z_real_e_imag(medidas_equivalentes, z_eq_tensao, nmed_BC,
    ...
    // x_bc = resolve_linear_QR(H_BC, z_eq, numeroRamos, nmed_BC);
    //
    delta_x_bc = resolve_linear_QR_Tensao(H_BC, H_T, z_eq_tensao, numeroRamos,
                                          nmed_BC, nmed_T, hx_I, hx_V);
    ...
    // mudar atualiza rede para receber complexo
    // atualiza_Rede_BC(grafo, numeroBarras, barra, regua_x, numeroRamos, x_bc);
    atualiza_Rede_BC_Tensao(grafo, numeroBarras, barra, regua_x, numeroRamos,
                            x_bc);

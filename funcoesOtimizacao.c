...
void atualiza_x(double *ponto, double *mu, double *x, long int nmed, long int nvar)
{
    long int i;
...
void IncCholeskyHachtelPCG(CCSPARSE *Hcc, CCSPARSE *Hcct, double **W, CCSPARSE *L, int nmed, int nvar)
{
    int i, j, k;
    double valor;
    SPARSE *tmp;
...
double NewtonMod_calcPasso(double alpha0, double *grad, double *dir, double *x, int n, double c1, GRAFO *grafo, long int numeroBarras, DMED *medidas, long int nmed, double *ponto, long int nvar, double *z, double **h, double **W, double *regua)
{
    int i = 0;
    int loop = 0;
    double passo = alpha0;
    ...
    do
    {
        ...
        printf("f(x) = %lf   f(x+1)=%lf   Armijo=%lf \t passo = %1f", fx, aux1, (fx - c1 * passo * aux0), passo);
        printf("\n");
        //getchar();
        //getchar();
        ...
        {
            trainspotting = ((-aux0 * passo * passo) / (2 * (aux1 - fx - passo * aux0)));
...
/*SPARSE* atualizaHsparse(DMED *medidas,long int nmed, double *regua, long int nvar){
    int i,j,r;
    SPARSE *Hs = NULL;
        ...
        for(j=0;j<medidas[i].nvar;j++){
            for(r = 0;r<nvar;r++){
                if (cabs(medidas[i].reguaH[j]-regua[r]) < EPS){
                    //Atualiza a Matriz H
                    if (medidas[i].H[j] != 0) sparseAdd(&Hs, i, r, medidas[i].H[j]);
                    break;
                }
            }
        }
...
void salva_sol(FILE *arqout, GRAFO *grafo, long int numeroBarras, double **h, double ***H, double **G, double *z, double *x, double *Dx, double *regua, int it, long int smed, long int nvar, double fx, double nGx, double tempo)
{
    ...
    for (i = 0; i < numeroBarras; i++)
    {
        ...
        //printf("%d\tVa: %.5lf + j%.5lf\tVb: %.5lf + j%.5lf\tVc: %.5lf + j%.5lf\n",grafo[i].barra->ID,__real__ grafo[i].V[0],__imag__ grafo[i].V[0],__real__ grafo[i].V[1],__imag__ grafo[i].V[1],__real__ grafo[i].V[2],__imag__ grafo[i].V[2]);
        //Polares
        fprintf(arqout, "%d\t%.5lf\t%.5lf\t%.5lf\t %.3lf\t%.3lf\t%.3lf\n", grafo[i].barra->ID, cabs(grafo[i].V[0] - grafo[i].V[1]), cabs(grafo[i].V[1] - grafo[i].V[2]), cabs(grafo[i].V[2] - grafo[i].V[0]), carg(grafo[i].V[0] - grafo[i].V[1]) * 180 / PI, carg(grafo[i].V[1] - grafo[i].V[2]) * 180 / PI, carg(grafo[i].V[2] - grafo[i].V[0]) * 180 / PI);
    }
}
...
int otimiza_Gauss_Newton(double *z, double **h, double ***H, double **W, GRAFO *grafo, long int numeroBarras, DRAM *ramos, DMED *medidas, long int nvar, long int nmed, double *regua_comp, double *ponto, double tol, long int ref1, long int ref2, int **Gsimb)
{
    ...
    //    for(i=0;i<nmed;i++)
    //    {
    //        fprintf(arquivo,"%.12lf\t %.12lf\t%.12lf\t%.12f\n",z[i],medidas[i].sigma,*h[i],z[i] - *h[i]);
    //    }
    ...
    while ((it < 30))
    {
        ...
        {
            //            if (it>=1){
            //                tira_refs(H,nvar+3,nmed,6,6,H_rf,regua,ponto,it); //Subestação
            //                nvar = nvar + 2;
            //            }
            //            else{
            //                tira_refs(H,nvar+3,nmed,6,8,H_rf,regua,ponto,it); //Subestação
        ...
        if (conv == 1)
        {
            ...
            fprintf(arquivo, "\n\n Convergência em %d iteracoes e tempo: %.4lf", it, tempoIt);
            salva_sol(arquivo, grafo, numeroBarras, h, H, G, z, ponto, Dx, regua, it, nmed, nvar, nFx, nGx, tempoIt);
        ...
        //        salva_sol(arquivo,grafo, numeroBarras, h, H, G, z, ponto, Dx, regua, it, nmed,nvar, nFx, nGx, 0);
        //        fprintf(arquivo,"\n\nVetor b \n");
        //        for(i=0;i<nvar;i++){
...
//------------------------------------------------------------------------------
//
//Método Clássico para a solução do Estimador WLS - Gauss Newton via QR factorization
//
int otimiza_Gauss_NewtonQR(double *z, double **h, double ***H, GRAFO *grafo, long int numeroBarras, DRAM *ramos, DMED *medidas, long int nvar, long int nmed, double *regua_comp, double *ponto, double tol, long int ref1, long int ref2)
{
    ...
    while ((it < 30))
    {
        ...
        X_SS = SuiteSparseQR_C_backslash(SPQR_ORDERING_BEST, SPQR_DEFAULT_TOL, A_SS, b_SS, c);
...
//Solução do problema de estimação de estado via gradiente conjugado para Least Squares lineares
//https://math.aalto.fi/opetus/inv/CGalgorithm.pdf
//int otimiza_CGLS_sparseNewton(double *z, double **h, double **W, GRAFO *grafo, long int numeroBarras, DRAM *ramos, DMED *medidas, long int nvar, long int nmed, double *regua, double *ponto, double tol, long int ref1, long int ref2){
...
//    arquivo = fopen("iteracoes.txt","w");
//
//    SPARSE *Ht = NULL, *Hs = NULL,*Cov = NULL,*Tmp = NULL, *M = NULL, *Ws = NULL;
//
//
//    //----------------------------------------------------------------------------
...
//    teste[0] = 1;
//    for(i=0;i<nmed;i++){
//        sparseAdd(&Cov, i, i, &W[i][i]);   //Matriz de Covariância ao invés da peso para o Hachtel - atualizar no valFun
//    }
//    // Matriz W esparsa
//    for(i=0;i<nmed;i++){
...
//    while(atual != NULL)
//    {
//        fprintf(arquivo2,"\n%d \t %d \t %.18lf",atual->i+1,atual->j+1,*atual->ij);
//        atual = atual->prox;
//    }
//    fprintf(arquivo2,"\nMatriz W:");
//    for(i=0;i<nmed;i++) fprintf(arquivo2,"\n%d \t %d \t %.18lf",i+1,i+1,W[i][i]);
//    fclose(arquivo2);
//
//    ///----------------------------------------------------------------------------
...
//        sparseMultMatVet(Ht, Dz, gradiente_n); //Tem que mudar para colocar a covariância diferente da identidade
//
//        for(i=0;i<nvar;i++) gradiente[i] = -gradiente_n[i];
//
//        double norma = norma_inf(gradiente,nvar);
////        printf("it: %d \t norma = %2f \t f(x) = %2f \n", it, norma,valorFuncao);
//
//        if ((norma <= tol)&&(it>=3)){
//            //Algoritmo convergiu
//            conv = 1;
...
//            //double** hess = hessianaAprox(); //Hachtel já vai estar atualizada
//
...
//
//            //double passo = NewtonMod_calcPasso(alpha0,gradiente,dir,ponto,nvar,Armijo_c1);
//            //double passo = NewtonMod_calcPasso(alpha0,gradiente,dir,Dx, nvar, Armijo_c1, grafo, numeroBarras, medidas, nmed, mu, ponto, nvar, z,h, Cov, regua);
...
//Matriz de Hachtel Aumentada - sem virtuais
int otimiza_Gauss_Newton_sparseHachtel(double *z, double **h, double **W, GRAFO *grafo, long int numeroBarras, DRAM *ramos, DMED *medidas, long int nvar, long int nmed, double *regua, double *ponto, double tol, long int ref1, long int ref2)
{
    long int it;
    long int NAV = 0, ref;
    optionHachtel = 1;
...
//Matriz de Hachtel Aumentada - sem virtuais
int otimiza_Gauss_Newton_sparsePCGLS(double *z, double **h, double ***H, double **W, GRAFO *grafo, long int numeroBarras, DRAM *ramos, DMED *medidas, long int nvar, long int nmed, double *regua, double *ponto, double tol, long int ref1, long int ref2)
{
    long int it;
    long int NAV = 0, ref;
    optionHachtel = 0;
...
//
        ...
        T_aux = cholmod_l_allocate_triplet(nmed, nvar, nmed*nvar, 0, CHOLMOD_REAL, c);
        Gaux = cholmod_l_allocate_sparse(nvar, nvar , nvar*nvar, 0, 0, 0, CHOLMOD_REAL, c);
        
        C_T = cholmod_l_allocate_triplet(nvar, nvir, nvar*nvir, 0, CHOLMOD_REAL, c);
        sC_T = cholmod_l_allocate_sparse(nvar, nvir , nvar*nvir, 0, 0, 0, CHOLMOD_REAL, c);
        
        Cn = cholmod_l_allocate_triplet(nvir, nvar+nvir, nvar*nvir, 0, CHOLMOD_REAL, c);
        sCn = cholmod_l_allocate_sparse(nvar, nvar+nvir , nvar*nvir, 0, 0, 0, CHOLMOD_REAL, c);
        
        T_tr = cholmod_l_allocate_triplet(nvar, nvar, nvar*nvar, 0, CHOLMOD_REAL, c);
        A_aux = cholmod_l_allocate_sparse(nmed, nvar , nmed*nvar, 0, 0, 0, CHOLMOD_REAL, c);
        A_T = cholmod_l_allocate_sparse(nvar, nmed , nmed*nvar, 0, 0, 0, CHOLMOD_REAL, c);

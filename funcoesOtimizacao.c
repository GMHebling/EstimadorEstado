...
//        free(aux);
        ...
        for(i=0;i<nmed;i++) Dz[i] = (z[i] - *h[i])/sqrt(W[i][i]);      
...
void montaGb(double **H,double **W, double **h, double *z,int nvar, int smed, double **G,double *b, int **Gsimb){
    ...
    for(i=0;i<nvar;i++){
        for(j=0;j<nvar;j++){
            G[i][j] = 0;
            for(k=0;k<smed;k++){
                if ((H[k][i] != 0) && (H[k][j] != 0)) G[i][j] += H[k][i]*W[k][k]*H[k][j];      
            }
        }
    }
//    for(i=0;i<nvar;i++){
...
//            }
            ...
            free(Dz);free(b);free(H_rf);free(Ht);free(G);free(Mtmp);free(regua);
            fclose(arquivo);
//            fclose(arqout);
            return conv;}
        //=========================================================================
...
//    }
    ...
    for(i=0;i<aux1_precon;i++) fscanf(precon,"%d",&permut[i]);
...
//            nvar = nvar +3;
        ...
        clock_t tHouse = clock();
        //Dx = solve_Householder(Hachtel,nvar + nmed,nvar + nmed,b_hach);
        for(i=0;i<nmed+nvar+nvir;i++) Dx[i] = 0;
        sparseCG_preCholesky(L,Hachtel,b_hach,Dx,nvar+nmed+nvir);

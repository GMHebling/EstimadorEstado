...
void imprimeTensoesNodais(GRAFO *grafo) {
  ...
  case 4:
    printf("%d\tVa: %.5lf | %.3lf \tVb: %.5lf | %.3lf\tVc:    -    |    -   \n",
           grafo->barra->ID, cabs(grafo->V[0]), carg(grafo->V[0]) * 180 / PI,
           cabs(grafo->V[1]), carg(grafo->V[1]) * 180 / PI);
    break;
  case 5:
    printf("%d\tVa: %.5lf | %.3lf \tVb:    -    |    -   \tVc: %.5lf | %.3lf\n",
           grafo->barra->ID, cabs(grafo->V[0]), carg(grafo->V[0]) * 180 / PI,
           cabs(grafo->V[2]), carg(grafo->V[2]) * 180 / PI);
    break;
  case 6:
  ...
  case 7:
    printf("%d\tVa: %.5lf | %.3lf \tVb: %.5lf | %.3lf\tVc: %.5lf | %.3lf\n",
           grafo->barra->ID, cabs(grafo->V[0]), carg(grafo->V[0]) * 180 / PI,
           cabs(grafo->V[1]), carg(grafo->V[1]) * 180 / PI, cabs(grafo->V[2]),
           carg(grafo->V[2]) * 180 / PI);
    break;
  }
}
...
void imprimeTaps(GRAFO *grafo) {
  ...
  for (i = 0; i < grafo->numeroAdjacentes; i++) {
    if ((grafo->adjacentes[i].tipo == regulador) &&
        (grafo->adjacentes[i].ramo->k == grafo->idNo)) {
      ...
      printf("%.3lf kVA\t%.2lf kV\tControl {%d}\t%.0lf\t%.0lf\t%.0lf\n",
             grafo->adjacentes[i].ramo->regulador.Snominal,
             grafo->adjacentes[i].ramo->regulador.Vnom / 1000,
             grafo->adjacentes[i].ramo->regulador.controle,
             grafo->adjacentes[i].ramo->regulador.tap[0],
             grafo->adjacentes[i].ramo->regulador.tap[1],
             grafo->adjacentes[i].ramo->regulador.tap[2]);
    }
  }
}
// Imprime na tela as tensões nodais trifásicas
void imprimeiInjecoesCorrentes(GRAFO *grafo) {
  ...
  case 1:
    printf("%d\tIa: %.5lf | %.3lf \tIb:    -    |    -   \tIc:    -    |    -  "
           " \n",
           grafo->barra->ID, cabs(grafo->Cur[0]),
           carg(grafo->Cur[0]) * 180 / PI);
    break;
  case 2:
    printf("%d\tIa:    -    |    -    \tIb: %.5lf | %.3lf\tIc:    -    |    -  "
           " \n",
           grafo->barra->ID, cabs(grafo->Cur[1]),
           carg(grafo->Cur[1]) * 180 / PI);
    break;
  case 3:
    printf("%d\tIa:    -    |    -    \tIb:    -    |    -   \tIc: %.5lf | "
           "%.3lf\n",
           grafo->barra->ID, cabs(grafo->Cur[2]),
           carg(grafo->Cur[2]) * 180 / PI);
    break;
  case 4:
    printf("%d\tIa: %.5lf | %.3lf \tIb: %.5lf | %.3lf\tIc:    -    |    -   \n",
           grafo->barra->ID, cabs(grafo->Cur[0]),
           carg(grafo->Cur[0]) * 180 / PI, cabs(grafo->Cur[1]),
           carg(grafo->Cur[1]) * 180 / PI);
    break;
  case 5:
    printf("%d\tIa: %.5lf | %.3lf \tIb:    -    |    -   \tIc: %.5lf | %.3lf\n",
           grafo->barra->ID, cabs(grafo->Cur[0]),
           carg(grafo->Cur[0]) * 180 / PI, cabs(grafo->Cur[2]),
           carg(grafo->Cur[2]) * 180 / PI);
    break;
  case 6:
    printf("%d\tIa:    -    |    -    \tIb: %.5lf | %.3lf\tIc: %.5lf | %.3lf\n",
           grafo->barra->ID, cabs(grafo->Cur[1]),
           carg(grafo->Cur[1]) * 180 / PI, cabs(grafo->Cur[2]),
           carg(grafo->Cur[2]) * 180 / PI);
    break;
  case 7:
...
void imprimeCorrentes(NOADJACENTE *noAdj) {
  ...
  case 1:
    printf("%d - %d\tIa: %.5lf | %.3lf \tIb:    -    |    -   \tIc:    -    |  "
           "  -   \n",
           noAdj->ramo->DE, noAdj->ramo->PARA, cabs(noAdj->Cur[0]),
           carg(noAdj->Cur[0]) * 180 / PI);
    break;
  case 2:
    printf("%d - %d\tIa:    -    |    -    \tIb: %.5lf | %.3lf\tIc:    -    |  "
           "  -   \n",
           noAdj->ramo->DE, noAdj->ramo->PARA, cabs(noAdj->Cur[1]),
           carg(noAdj->Cur[1]) * 180 / PI);
    break;
  case 3:
    printf("%d - %d\tIa:    -    |    -    \tIb:    -    |    -   \tIc: %.5lf "
           "| %.3lf\n",
           noAdj->ramo->DE, noAdj->ramo->PARA, cabs(noAdj->Cur[2]),
           carg(noAdj->Cur[2]) * 180 / PI);
    break;
  case 4:
    printf("%d - %d\tIa: %.5lf | %.3lf \tIb: %.5lf | %.3lf\tIc:    -    |    - "
           "  \n",
           noAdj->ramo->DE, noAdj->ramo->PARA, cabs(noAdj->Cur[0]),
           carg(noAdj->Cur[0]) * 180 / PI, cabs(noAdj->Cur[1]),
           carg(noAdj->Cur[1]) * 180 / PI);
    break;
  case 5:
    printf("%d - %d\tIa: %.5lf | %.3lf \tIb:    -    |    -   \tIc: %.5lf | "
           "%.3lf\n",
           noAdj->ramo->DE, noAdj->ramo->PARA, cabs(noAdj->Cur[0]),
           carg(noAdj->Cur[0]) * 180 / PI, cabs(noAdj->Cur[2]),
           carg(noAdj->Cur[2]) * 180 / PI);
    break;
  case 6:
    printf("%d - %d\tIa:    -    |    -    \tIb: %.5lf | %.3lf\tIc: %.5lf | "
           "%.3lf\n",
           noAdj->ramo->DE, noAdj->ramo->PARA, cabs(noAdj->Cur[1]),
           carg(noAdj->Cur[1]) * 180 / PI, cabs(noAdj->Cur[2]),
           carg(noAdj->Cur[2]) * 180 / PI);
    break;
  case 7:
    printf(
        "%d - %d\tIa: %.5lf | %.3lf \tIb: %.5lf | %.3lf\tIc: %.5lf | %.3lf\n",
        noAdj->ramo->DE, noAdj->ramo->PARA, cabs(noAdj->Cur[0]),
        carg(noAdj->Cur[0]) * 180 / PI, cabs(noAdj->Cur[1]),
        carg(noAdj->Cur[1]) * 180 / PI, cabs(noAdj->Cur[2]),
        carg(noAdj->Cur[2]) * 180 / PI);
...
// Função inicialização do vetor x para sistemas radiais (sem o arquivo
// Vinicial)
void incializaTensoesRaiz(GRAFO *grafo, long int numeroBarras,
                          ALIMENTADOR *alimentadores,
                          long int numeroAlimentadores) {
  int i, k, fase;
...
// Função inicialização do vetor x para sistemas radiais (sem o arquivo
// Vinicial)
void incializaTensoesVarredura(GRAFO *grafo, long int numeroBarras,
                               ALIMENTADOR *alimentadores,
                               long int numeroAlimentadores) {
  int i, k, fase;
  BOOL visitado[numeroBarras];
...
void atualizaTapRegulador(DRAM *ramo) {
  if (ramo->tipo == 2) {
    ...
    ramo->Yps[0][0] =
        1 / ramo->tap_sec[0] * 1 / ramo->tap_pri[0] * ramo->Yps[0][0];
    ramo->Yps[1][1] =
        1 / ramo->tap_sec[1] * 1 / ramo->tap_pri[1] * ramo->Yps[1][1];
    ramo->Yps[2][2] =
        1 / ramo->tap_sec[2] * 1 / ramo->tap_pri[2] * ramo->Yps[2][2];

    ramo->Ysp[0][0] =
        1 / ramo->tap_sec[0] * 1 / ramo->tap_pri[0] * ramo->Ysp[0][0];
    ramo->Ysp[1][1] =
        1 / ramo->tap_sec[1] * 1 / ramo->tap_pri[1] * ramo->Ysp[1][1];
    ramo->Ysp[2][2] =
        1 / ramo->tap_sec[2] * 1 / ramo->tap_pri[2] * ramo->Ysp[2][2];
    ...
    // Atualiaza os taps
    ramo->tap_pri[0] = (1 + ramo->regulador.tap[0] * ramo->regulador.regulacao /
                                ramo->regulador.ntaps);
    ramo->tap_pri[1] = (1 + ramo->regulador.tap[1] * ramo->regulador.regulacao /
                                ramo->regulador.ntaps);
    ramo->tap_pri[2] = (1 + ramo->regulador.tap[2] * ramo->regulador.regulacao /
                                ramo->regulador.ntaps);
...
// Função que atualiza as injeções de corrente nas barras da rede conforme as
// cargas e e respectivos modelos de carga, shunts e geradores distribuidos
void atualizaInjecoes(GRAFO *no) {
  ...
  for (i = 0; i < no->barra->nloads; i++) {
    ...
    case YN:
      Saux[0] =
          (no->barra->loads[i].Pnom[0] + I * no->barra->loads[i].Qnom[0]) *
          pow(cabs(no->V[0]), no->barra->loads[i].ZIP);
      Saux[1] =
          (no->barra->loads[i].Pnom[1] + I * no->barra->loads[i].Qnom[1]) *
          pow(cabs(no->V[1]), no->barra->loads[i].ZIP);
      Saux[2] =
          (no->barra->loads[i].Pnom[2] + I * no->barra->loads[i].Qnom[2]) *
          pow(cabs(no->V[2]), no->barra->loads[i].ZIP);
    ...
    case Y:
      ...
      Saux[0] =
          (no->barra->loads[i].Pnom[0] + I * no->barra->loads[i].Qnom[0]) *
          pow(cabs(no->V[0] - V0), no->barra->loads[i].ZIP);
      Saux[1] =
          (no->barra->loads[i].Pnom[1] + I * no->barra->loads[i].Qnom[1]) *
          pow(cabs(no->V[1] - V0), no->barra->loads[i].ZIP);
      Saux[2] =
          (no->barra->loads[i].Pnom[2] + I * no->barra->loads[i].Qnom[2]) *
          pow(cabs(no->V[2] - V0), no->barra->loads[i].ZIP);
  ...
  for (i = 0; i < no->barra->nshunts; i++) {
    ...
    case Y:
      V0 = no->V[0] + no->V[1] + no->V[2]; // tensão de neutro
      Saux[0] =
          (I * no->barra->shunts[i].Qnom[0]) * pow(cabs(no->V[0] - V0), 2);
      Saux[1] =
          (I * no->barra->shunts[i].Qnom[1]) * pow(cabs(no->V[1] - V0), 2);
      Saux[2] =
          (I * no->barra->shunts[i].Qnom[2]) * pow(cabs(no->V[2] - V0), 2);
...
void calculaTensaoJusante(__complex__ double *Vp, __complex__ double *Vs,
                          __complex__ double *Ips, DRAM *ramo) {
  ...
  case 1: // Trafo - está para o YN-YN
    ...
    // tratamento de excessões das ligações de trafos com matriz de admitância
    // singulares
    //--------------------------------------
    ...
    if (singular1) {
      ...
      for (i = 0; i < 3; i++) {
        ...
        Aux[i] = -Vaux[i];
        //  Aux[i] = Isp[i] - Vaux[i]; // Falta pensar na corrente do
        //  secundário!
      }
      for (i = 0; i < 3; i++) {
        Vs[i] = 0;
      ...
      V0 = 0; // Aproximação para caso de ligação em Delta no secundário - ver
              // referência (Tratativa específica para calcular V0 no secundario
    ...
    for (i = 0; i < 3; i++) {
      ...
      Vs[i] += V0;
      if (cabs(Vs[i]) == 0)
        Vs[i] = 1; // Caso em que não tem a fase resulta em V = 0 - coloca 1
                   // para evitar NaN na hora de atualizar a corrente
    }
  ...
  if (noMont != -1) {
    ...
    case 0: // Linha
      ...
      for (i = 0; i < 3; i++) {
        ...
        for (j = 0; j < 3; j++) {
          Iaux[i] +=
              noP->adjacentes[auxNoMont].ramo->B[i][j] * noP->V[j] +
              noP->adjacentes[auxNoMont].ramo->B[i][j] * grafo[noMont].V[j];
        }
      }
    ...
    case 1: // Trafo
      ...
      for (i = 0; i < 3; i++) {
        ...
        Vaux[i] += V0; // quando é Ysp singular soma a tensão de sequência zero
                       // - se não for singular V0 = 0
      }
      ...
      for (i = 0; i < 3; i++) {
        Iacc[i] = 0;
        for (j = 0; j < 3; j++) {
          Iacc[i] += noP->adjacentes[auxNoMont].ramo->Ypp[i][j] * Vaux[j] +
                     noP->adjacentes[auxNoMont].ramo->Yps[i][j] * noP->V[j];
        }
      }
      break;
    //------------------------------------------------------------------
...
// Função que atualiza controle de reguladores de tensão de acordo com
// parametrização do LDC
BOOL controleReguladorTensao_LDC(double Vbase, double Ibase, complex double *Vp,
                                 complex double *Vs, complex double *Ips,
                                 complex double *Isp, DRAM *ramo) {
  int i;
  ...
  for (i = 0; i < 3; i++)
    taps_0[i] = ramo->regulador.tap[i];

  // Calcula direção do fluxo
  for (i = 0; i < 3; i++) {
    if ((creal(Vs[i] * conj(Isp[i])) && (cimag(Vs[i] * conj(Isp[i])) >= 0)) >=
        0)
      Direction[i] = 1;
    else
      Direction[i] = 0;
  ...
  case 0:
    for (i = 0; i < 3; i++)
      Vdiff[i] = Vset[i] - cabs(Vbase * Vs[i] / ramo->regulador.TP -
                                Zcont[i] * Ibase / sqrt(3) * (-Isp[i]) /
                                    ramo->regulador.TC);
    ...
    for (i = 0; i < 3; i++) {
      ...
      if (Vdiff[i] >= ramo->regulador.deltaV)
        ramo->regulador.tap[i] =
            ramo->regulador.tap[i] + ceil((Vdiff[i] - ramo->regulador.deltaV) /
                                          (reg * ramo->regulador.Vnom /
                                           pow(3, 0.5) / ramo->regulador.TP));
      else if (Vdiff[i] <= -ramo->regulador.deltaV)
        ramo->regulador.tap[i] =
            ramo->regulador.tap[i] + floor((Vdiff[i] + ramo->regulador.deltaV) /
                                           (reg * ramo->regulador.Vnom /
                                            pow(3, 0.5) / ramo->regulador.TP));
      // printf("\t tap[%d]= %lf",i, ramo->regulador.tap[i]);
    }
  ...
  case 1:
    for (i = 0; i < 3; i++)
      Vdiff[i] =
          Vset[i] - cabs(Vbase * Vs[i] / ramo->regulador.TP -
                         Zcont[i] * Ibase * (-Isp[i]) / ramo->regulador.TC);

    for (i = 0; i < 3; i++) {
      if (Direction[i] == 1) {
        if (Vdiff[i] >= ramo->regulador.deltaV)
          ramo->regulador.tap[i] = ramo->regulador.tap[i] +
                                   ceil((Vdiff[i] - ramo->regulador.deltaV) *
                                        (reg * ramo->regulador.Vnom /
                                         pow(3, 0.5) / ramo->regulador.TP));
        else if (Vdiff[i] <= -ramo->regulador.deltaV)
          ramo->regulador.tap[i] = ramo->regulador.tap[i] +
                                   floor((Vdiff[i] + ramo->regulador.deltaV) *
                                         (reg * ramo->regulador.Vnom /
                                          pow(3, 0.5) / ramo->regulador.TP));
      } else {
  ...
  case 2:
    for (i = 0; i < 3; i++)
      Vdiff[i] =
          Vset_r[i] - cabs(Vbase * Vp[i] / ramo->regulador.TP -
                           Zcont_r[i] * Ibase * (-Ips[i]) / ramo->regulador.TC);
    ...
    for (i = 0; i < 3; i++) {
      if (Direction[i] == 1) {
        ...
        if (Vdiff[i] >= ramo->regulador.deltaVr)
          ramo->regulador.tap[i] = ramo->regulador.tap[i] -
                                   ceil((Vdiff[i] - ramo->regulador.deltaVr) *
                                        (reg * ramo->regulador.Vnom /
                                         pow(3, 0.5) / ramo->regulador.TP));
        else if (Vdiff[i] <= -ramo->regulador.deltaVr)
          ramo->regulador.tap[i] = ramo->regulador.tap[i] -
                                   floor((Vdiff[i] + ramo->regulador.deltaVr) *
                                         (reg * ramo->regulador.Vnom /
                                          pow(3, 0.5) / ramo->regulador.TP));
      }
    }
  ...
  case 3:
    for (i = 0; i < 3; i++) {
      if (Direction[i] == 1)
        Vdiff[i] = Vset[i] - cabs(Vs[i] / ramo->regulador.TP -
                                  Zcont[i] * Isp[i] / ramo->regulador.TC);
      else
        Vdiff[i] = Vset_r[i] - cabs(Vp[i] / ramo->regulador.TP -
                                    Zcont_r[i] * Ips[i] / ramo->regulador.TC);
    }
  ...
  case 6:
    for (i = 0; i < 3; i++) {
      if (Direction[i] == 1)
        ...
        Vdiff[i] = Vset[i] - cabs(Vs[i] / ramo->regulador.TP -
                                  Zcont_r[i] * Isp[i] / ramo->regulador.TC);
    }
  ...
  for (i = 0; i < 3; i++) {
    ...
    else if ((ramo->regulador.tap[i] < -ramo->regulador.ntaps) &&
             (Direction[i] == 1))
      ramo->regulador.tap[i] = -ramo->regulador.ntaps;
    else if ((ramo->regulador.tap[i] > ramo->regulador.ntaps) &&
             (Direction[i] == -1))
  ...
  ctr_act = 0;
  for (i = 0; i < 3; i++) {
    if (ramo->regulador.tap[i] != taps_0[i])
      ctr_act = 1;
  }
...
BOOL forward_sweep(GRAFO *noP, GRAFO *grafo) {
  ...
  BOOL control_action =
      0; // variável para indicar se houve transição por parte dos controladores
  double Sbase;
  double Ibase;

  BOOL control_REG_OPT = 0;
  BOOL control_CAP_OPT = 0;
  ...
  for (i = 0; i < noP->numeroAdjacentes; i++) {
    ...
    if (noP->profundidade < grafo[noAdj].profundidade) {
      ...
      calculaTensaoJusante(noP->V, Vaux, noP->adjacentes[i].Cur,
                           noP->adjacentes[i].ramo);
      ...
      if (control_REG_OPT == 1) {
        if (noP->adjacentes[i].tipo == 2) {
          ...
          control_action = controleReguladorTensao_LDC(
              noP->Vbase, Sbase / noP->Vbase, noP->V, Vaux,
              noP->adjacentes[i].Cur, grafo[noAdj].adjacentes[idx].Cur,
              noP->adjacentes[i].ramo);
          // printf("\nLDC %d\n",control_action);
        }
      }
      //------------------------------------------------------------------
      // Atualiza Bancos de Capacitor Chaveados com os controladores de Bancos
      // de Capacitor
      if (control_CAP_OPT == 1) {
      }
      // //------------------------------------------------------------------
      // //Atualiza Geradores Distribuídos com os controladores de Tensão
      // (Futuro) if (control_GD_OPT == 1){
...
int fluxoPotencia_BFS_Alimentador(GRAFO *grafo, long int numeroBarras,
                                  ALIMENTADOR alimentador, DRAM *ramos,
                                  double Sbase) {
...
void fluxoPotencia_BFS_Multiplos(GRAFO *grafo, long int numeroBarras,
                                 ALIMENTADOR *alimentadores,
  ...
  incializaTensoesVarredura(grafo, numeroBarras, alimentadores,
                            numeroAlimentadores);
  //    incializaTensoesRaiz(grafo, numeroBarras, alimentadores,
  //    numeroAlimentadores);
  ...
  for (idAlim = 0; idAlim < numeroAlimentadores; idAlim++) {
    it = fluxoPotencia_BFS_Alimentador(grafo, numeroBarras,
                                       alimentadores[idAlim], ramos, Sbase);

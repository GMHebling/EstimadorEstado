...
char *leituraDados(DBAR **barra, DRAM **ramo, long int *numeroBarras,
                   long int *numeroRamos, long int *numeroAlimentadores) {
  FILE *arquivo = NULL;
  ...
  //    //scanf("%s",pasta);
  //    //strcpy(folder,"C:/Users/Julio
  //    Massignan/Dropbox/Doutorado/Codigos/Estimador_Trifasico/");
  //    strcpy(folder,"C:/Users/Julio/Dropbox/Doutorado/Codigos/Estimador_Trifasico/");
  //    strcpy(pasta,"IEEE4");
  //    //strcpy(folder,"C:/Users/Julio
  //    Massignan/Dropbox/Doutorado/Codigos/Simulador_SD/IEEE34");
  // printf("TESTE\n");
  // strcat(folder,"/");
  // strcat(folder,pasta);
  // strcat(folder,"/");
  ...
  strcpy(aux, aux2);
  arquivo =
      fopen(strcat(aux, "DSHNT.csv"), "r"); // Le somente se existir o arquivo
  if (arquivo != NULL) {
  ...
  strcpy(aux, aux2);
  arquivo =
      fopen(strcat(aux, "DGD.csv"), "r"); // Le somente se existir o arquivo
  if (arquivo != NULL) {
  ...
  strcpy(aux, aux2);
  arquivo =
      fopen(strcat(aux, "DLIN.csv"), "r"); // Le somente se existir o arquivo
  if (arquivo != NULL) {
  ...
  strcpy(aux, aux2);
  arquivo =
      fopen(strcat(aux, "DTRF.csv"), "r"); // Le somente se existir o arquivo
  if (arquivo != NULL) {
  ...
  strcpy(aux, aux2);
  arquivo =
      fopen(strcat(aux, "DREG.csv"), "r"); // Le somente se existir o arquivo
  if (arquivo != NULL) {
  ...
  strcpy(aux, aux2);
  arquivo =
      fopen(strcat(aux, "DSWTC.csv"), "r"); // Le somente se existir o arquivo
  if (arquivo != NULL) {
  ...
  strcpy(aux, aux2);
  arquivo = fopen(strcat(aux, "Vinicial.csv"),
                  "r"); // Le somente se existir o arquivo
  if (arquivo != NULL) {
...
void leituraDBAR(FILE *arquivo, DBAR **barras, long int *numeroBarras,
                 long int *numeroAlimentadores) {
  char blocoLeitura[2000]; /* Variável para realizar a leitura do bloco de
                              caracteres do arquivo. */
  char *dados; /* Variável do tipo ponteiro para char, utilizada para alterar o
                  ponteiro da string lida do arquivo de forma a realizar o loop
                  no sscanf. */
  int contador = 0, i, aux, k; /* Variáveis contadores para percorrer o arquivo
                                  e a string de leitura. */
  int carac,
      numLinhas =
  ...
  while ((fgets(blocoLeitura, 2000, arquivo)) != NULL) {
    ...
    if (aux == -1) { // Criando novo DBAR
      ...
      (*barras)[contador].Vinicial[1] =
          1 * (cos(-120 * PI / 180) + I * sin(-120 * PI / 180));
      (*barras)[contador].Vinicial[2] =
          1 * (cos(120 * PI / 180) + I * sin(120 * PI / 180));
      ...
      if ((PA != 0) || (PB != 0) || (PC != 0) || (QA != 0) || (QB != 0) ||
        ...
        /*if (((*barras)[contador].loads = (DLOAD
        *)realloc((*barras)[contador].loads, ((*barras)[contador].nloads +1) *
        sizeof(DLOAD)))==NULL)
        {
      ...
      if (getfield(dados, 13) != NULL) {
        ...
        (*barras)[contador].Vinicial[0] =
            VA * (cos(TA * M_PI / 180) + I * sin(TA * M_PI / 180));
        (*barras)[contador].Vinicial[1] =
            VB * (cos(TB * M_PI / 180) + I * sin(TB * M_PI / 180));
        (*barras)[contador].Vinicial[2] =
            VC * (cos(TC * M_PI / 180) + I * sin(TC * M_PI / 180));
      }
      contador++;
      ...
      if ((PA != 0) || (PB != 0) || (PC != 0) || (QA != 0) || (QB != 0) ||
        ...
        // Inseri um valor de load
        (*barras)[aux].nloads++;
        /*if (((*barras)[aux].loads = (DLOAD *)realloc((*barras)[aux].loads,
        (*barras)[aux].nloads * sizeof(DLOAD)))==NULL)
        {
            printf("Erro -- Nao foi possivel alocar espaco de memoria para
        cargas !!!!"); exit(1);
        }*/
...
void leituraDSHNT(FILE *arquivo, DBAR **barras, long int *numeroBarras) {
  ...
  char *dados; /* Variável do tipo ponteiro para char, utilizada para alterar o
                  ponteiro da string lida do arquivo de forma a realizar o loop
                  no sscanf. */
  int i, aux, k; /* Variáveis contadores para percorrer o arquivo e a string de
                    leitura. */
  dados = (char *)malloc(100);
  // Le o arquivo de curva de cargas até o fim
  while ((fgets(blocoLeitura, 2000, arquivo)) != NULL) {
    ...
    if (aux != -1) {
      (*barras)[aux].nshunts++;
      if (((*barras)[aux].shunts = (DSHNT *)realloc(
               (*barras)[aux].shunts,
...
void leituraDGD(FILE *arquivo, DBAR **barras, long int *numeroBarras) {
  ...
  while ((fgets(blocoLeitura, 2000, arquivo)) != NULL) {
    ...
    if (aux != -1) {
      (*barras)[aux].ngds++;
      if (((*barras)[aux].gds =
               (DGD *)realloc((*barras)[aux].gds,
                              (*barras)[aux].ngds * sizeof(DGD))) == NULL) {
        printf(
            "Erro -- Nao foi possivel alocar espaco de memoria para gds !!!!");
        exit(1);
      }
      k = (*barras)[aux].ngds - 1;
...
void leituraDLIN(FILE *arquivo, DRAM **ramos, long int *numeroRamos,
                 DBAR **barras, long int *numeroBarras) {
  char blocoLeitura[2000]; /* Variável para realizar a leitura do bloco de
                              caracteres do arquivo. */
  char *dados; /* Variável do tipo ponteiro para char, utilizada para alterar o
                  ponteiro da string lida do arquivo de forma a realizar o loop
                  no sscanf. */
  long int contador = 0, i, aux; /* Variáveis contadores para percorrer o
...
void leituraDTRF(FILE *arquivo, DRAM **ramos, long int *numeroRamos,
                 DBAR **barras, long int *numeroBarras) {
  char blocoLeitura[2000]; /* Variável para realizar a leitura do bloco de
                              caracteres do arquivo. */
  char *dados; /* Variável do tipo ponteiro para char, utilizada para alterar o
                  ponteiro da string lida do arquivo de forma a realizar o loop
                  no sscanf. */
  int contador = 0, i, aux; /* Variáveis contadores para percorrer o arquivo e a
                               string de leitura. */
  int carac,
      numLinhas =
          0; /* Variável com o número de linhas do arquivo a serem lidas. */
  ...
  while ((fgets(blocoLeitura, 2000, arquivo)) != NULL) {
    ...
    (*ramos)[contador].trafo.R =
        atof(getfield(dados, 7)) * pow((*ramos)[contador].trafo.Vsec, 2) /
        ((*ramos)[contador].trafo.Snominal * 1000) / 3; // R*(Vpri^2/(S*1000))
    (*ramos)[contador].trafo.X = atof(getfield(dados, 8)) *
                                 pow((*ramos)[contador].trafo.Vsec, 2) /
                                 ((*ramos)[contador].trafo.Snominal * 1000) / 3;
...
void leituraDREG(FILE *arquivo, DRAM **ramos, long int *numeroRamos,
                 DBAR **barras, long int *numeroBarras) {
  char blocoLeitura[2000]; /* Variável para realizar a leitura do bloco de
                              caracteres do arquivo. */
  char *dados; /* Variável do tipo ponteiro para char, utilizada para alterar o
  ...
  while ((fgets(blocoLeitura, 2000, arquivo)) != NULL) {
    ...
    (*ramos)[contador].regulador.R =
        atof(getfield(dados, 8)) * pow((*ramos)[contador].regulador.Vnom, 2) /
        ((*ramos)[contador].regulador.Snominal * 1000) / 3;
    (*ramos)[contador].regulador.X =
        atof(getfield(dados, 9)) * pow((*ramos)[contador].regulador.Vnom, 2) /
        ((*ramos)[contador].regulador.Snominal * 1000) / 3;
...
void leituraDSWTC(FILE *arquivo, DRAM **ramos, long int *numeroRamos,
                  DBAR **barras, long int *numeroBarras) {
  char blocoLeitura[2000]; /* Variável para realizar a leitura do bloco de
                              caracteres do arquivo. */
  char *dados; /* Variável do tipo ponteiro para char, utilizada para alterar o
...
long int **leituraMedidas(char *folder, char *file, DMED **medidas, DRAM *ramos,
                          long int numeroRamos, DBAR *barras,
                          long int numeroBarras, GRAFO *grafo, double Sbase) {
  char blocoLeitura[2000]; /* Variável para realizar a leitura do bloco de
                              caracteres do arquivo. */
  char *dados; /* Variável do tipo ponteiro para char, utilizada para alterar o
                  ponteiro da string lida do arquivo de forma a realizar o loop
                  no sscanf. */
  long int contador = 0, i, j, k, l, m, ind, aux,
           adj; /* Variáveis contadores para percorrer o arquivo e a string de
                   leitura. */
  int carac,
      numLinhas =
          0; /* Variável com o número de linhas do arquivo a serem lidas. */
  ...
  while ((fgets(blocoLeitura, 2000, arquivo)) != NULL) {
    ...
    case 13: // PMU de corrente retangular imaginário
      for (i = 0; i < numeroRamos; i++) {
        if (((*medidas)[contador].DE == ramos[i].DE) &&
            ((*medidas)[contador].PARA == ramos[i].PARA)) {
          (*medidas)[contador].k = ramos[i].k;
          (*medidas)[contador].m = ramos[i].m;
          (*medidas)[contador].ramo = i;
        }
        if (((*medidas)[contador].DE == ramos[i].PARA) &&
            ((*medidas)[contador].PARA == ramos[i].DE)) {
  ...
  for (i = 0; i < nmed; i++) {
    ...
    case 0: // Medida de Fluxo de Potência Ativa em kW
      ...
      (*medidas)[i].nvar = 12;
      (*medidas)[i].reguaH =
          (double *)malloc((*medidas)[i].nvar * sizeof(double));
    ...
    case 10: // Medida PMU de injeção de corrente retangular real
      Vbase = grafo[k].Vbase;
      (*medidas)[i].zmed =
          (*medidas)[i].zmed / ((Sbase / 1000) / ((Vbase / 1000)));
      (*medidas)[i].sigma =
          (*medidas)[i].sigma / ((Sbase / 1000) / ((Vbase / 1000)));
  ...
  for (i = 0; i < nmed; i++) {
    ...
    case 0: // Medida de Fluxo de Potência Ativa em kW
      for (j = 0; j < nmed; j++) {
        if (((*medidas)[j].tipo == 1) && ((*medidas)[j].k == k) &&
            ((*medidas)[j].m == m)) {
          (*medidas)[i].par = j;
          (*medidas)[j].par = i;
          j = nmed;
    ...
    case 2: // Medida de Injeção de Potência Ativa em kW
      for (j = 0; j < nmed; j++) {
        if (((*medidas)[j].tipo == 3) && ((*medidas)[j].k == k) &&
            ((*medidas)[j].m == m)) {
          (*medidas)[i].par = j;
          (*medidas)[j].par = i;
          j = nmed;
    ...
    case 4: // Medida de Magnitude de Tensão - kV
      for (j = 0; j < nmed; j++) {
        if (((*medidas)[j].tipo == 5) && ((*medidas)[j].k == k) &&
            ((*medidas)[j].m == m)) {
          (*medidas)[i].par = j;
          (*medidas)[j].par = i;
          j = nmed;
    ...
    case 6: // Medida de Magnitude de Corrente em A
      for (j = 0; j < nmed; i++) {
        if (((*medidas)[j].tipo == 7) && ((*medidas)[j].k == k) &&
            ((*medidas)[j].m == m)) {
          (*medidas)[i].par = j;
          (*medidas)[j].par = i;
          j = nmed;
...
void geraGrafo(GRAFO **grafo, DBAR *barras, long int numeroBarras, DRAM *ramos,
               long int numeroRamos) {
  long int i, j, k;
  ...
  for (i = 0; i < numeroRamos; i++) {
    ...
    if (ramos[i].tipo == 1) {
      (*grafo)[barraDe].adjacentes[nadj].relacao =
          ramos[i].trafo.Vsec / ramos[i].trafo.Vpri;
    }
    (*grafo)[barraDe].adjacentes[nadj].medidores =
    ...
    if (ramos[i].tipo == 1) {
      (*grafo)[barraPara].adjacentes[nadj].relacao =
          ramos[i].trafo.Vpri / ramos[i].trafo.Vsec;
    }
    (*grafo)[barraPara].adjacentes[nadj].medidores =
        (DMED **)malloc(30 * sizeof(DMED *));
  }
}
...
void salvaDadosRedeEletrica(DBAR *barras, long int numeroBarras, DRAM *ramos,
                            long int numeroRamos, DMED *medidas,
  ...
  for (i = 0; i < numeroBarras; i++) {
    fprintf(arquivo, "Barra: %d\t%d\t%s\t%.2lf", barras[i].i, barras[i].ID,
            charFases(barras[i].fases), barras[i].Vbase);
    if (barras[i].nloads > 0) {
      for (j = 0; j < barras[i].nloads; j++)
        fprintf(arquivo,
                "\n\tCarga "
                "%d:\t%s\t%.5lf\t%.5lf\t%.5lf\t%.5lf\t%.5lf\t%.5lf\t%.1lf",
                j, charLigacao(barras[i].loads[j].lig),
                barras[i].loads[j].Pnom[0], barras[i].loads[j].Pnom[1],
                barras[i].loads[j].Pnom[2], barras[i].loads[j].Qnom[0],
                barras[i].loads[j].Qnom[1], barras[i].loads[j].Qnom[2],
                barras[i].loads[j].ZIP);
    }
    if (barras[i].nshunts > 0) {
      for (j = 0; j < barras[i].nshunts; j++) {
        fprintf(arquivo, "\n\tShunt %d:\t%s\t%.5lf\t%.5lf\t%.5lf\t%d", j,
                charLigacao(barras[i].shunts[j].lig),
                barras[i].shunts[j].Qnom[0], barras[i].shunts[j].Qnom[1],
                barras[i].shunts[j].Qnom[2], barras[i].shunts[j].controle);
        if (barras[i].shunts[j].controle != 0)
          fprintf(arquivo, "\t%.5lf\t%.5lf\t%.5lf\t%.5lf",
                  barras[i].shunts[j].DV, barras[i].shunts[j].Vset[0],
                  barras[i].shunts[j].Vset[1], barras[i].shunts[j].Vset[2]);
      }
    }
    if (barras[i].ngds > 0) {
      for (j = 0; j < barras[i].ngds; j++) {
        fprintf(arquivo,
                "\n\tGD %d:\t%s\t%.5lf "
                "kVA\t%.5lf\t%.5lf\t%.5lf\t%.5lf\t%.5lf\t%.5lf\t%.1lf",
                j, charLigacao(barras[i].gds[j].lig), barras[i].gds[j].Snominal,
                barras[i].gds[j].Pnom[0], barras[i].gds[j].Pnom[1],
                barras[i].gds[j].Pnom[2], barras[i].gds[j].Qnom[0],
                barras[i].gds[j].Qnom[1], barras[i].gds[j].Qnom[2],
                barras[i].gds[j].controle);
        if (barras[i].gds[j].controle != 0)
          fprintf(arquivo, "\t%.5lf\t%.5lf\t%.5lf\t%.5lf\t%.5lf",
                  barras[i].gds[j].Qmin, barras[i].gds[j].Qmax,
                  barras[i].gds[j].Vset[0], barras[i].gds[j].Vset[1],
                  barras[i].gds[j].Vset[2], barras[i].gds[j].controlePV);
      }
    }
    fprintf(arquivo, "\n");
  }
  ...
  for (i = 0; i < numeroRamos; i++) {
    switch (ramos[i].tipo) {
    case ramal:
      fprintf(arquivo, "Linha %d (%d)\t%d (%d):\t%s\t%d", ramos[i].DE,
              ramos[i].k, ramos[i].PARA, ramos[i].m, charFases(ramos[i].fases),
              ramos[i].estado);
      fprintf(arquivo, "\n\t%.4lf  %.4lf\t\t%.4lf  %.4lf\t\t%.4lf  %.4lf",
              __real__ ramos[i].linha.Zaa, __imag__ ramos[i].linha.Zaa,
              __real__ ramos[i].linha.Zab, __imag__ ramos[i].linha.Zab,
              __real__ ramos[i].linha.Zac, __imag__ ramos[i].linha.Zac);
      fprintf(arquivo, "\n\t\t\t\t\t%.4lf  %.4lf\t\t%.4lf  %.4lf",
              __real__ ramos[i].linha.Zbb, __imag__ ramos[i].linha.Zbb,
              __real__ ramos[i].linha.Zbc, __imag__ ramos[i].linha.Zbc);
      fprintf(arquivo, "\n\t\t\t\t\t\t\t\t\t%.4lf  %.4lf",
              __real__ ramos[i].linha.Zcc, __imag__ ramos[i].linha.Zcc);
      break;
    case trafo:
      fprintf(arquivo, "Trafo %d (%d)\t%d (%d):\t%s\t%d", ramos[i].DE,
              ramos[i].k, ramos[i].PARA, ramos[i].m, charFases(ramos[i].fases),
              ramos[i].estado);
      fprintf(arquivo,
              "\n\t%.4lf  %.4lf\t%.3lf\t%.2lf / %.2lf\t%s%s%d\t%.4lf\t%.4lf",
              ramos[i].trafo.R, ramos[i].trafo.X, ramos[i].trafo.Snominal,
              ramos[i].trafo.Vpri, ramos[i].trafo.Vsec,
              charLigacao(ramos[i].trafo.lig_pri),
              charLigacao(ramos[i].trafo.lig_sec), ramos[i].trafo.defasamento,
              ramos[i].trafo.tap_pri, ramos[i].trafo.tap_sec);
      break;
    case regulador:
      fprintf(arquivo, "Regulador %d (%d)\t%d (%d):\t%s\t%d", ramos[i].DE,
              ramos[i].k, ramos[i].PARA, ramos[i].m, charFases(ramos[i].fases),
              ramos[i].estado);
      fprintf(arquivo,
              "\n\t%.4lf  %.4lf\t%.3lf\t%.2lf\t%s\t%d\t%.4lf\t%.4lf\t%.4lf",
              ramos[i].regulador.R, ramos[i].regulador.X,
              ramos[i].regulador.Snominal, ramos[i].regulador.Vnom,
              charLigacao(ramos[i].regulador.lig), ramos[i].regulador.controle,
              ramos[i].regulador.tap[0], ramos[i].regulador.tap[1],
              ramos[i].regulador.tap[2]);
      break;
    case chave:
      fprintf(arquivo, "Chave %d (%d)\t%d (%d):\t%s\t%d", ramos[i].DE,
              ramos[i].k, ramos[i].PARA, ramos[i].m, charFases(ramos[i].fases),
              ramos[i].estado);
      break;
    }
  ...
  fprintf(arquivo, "\nLISTA DE MEDIDORES:\n");
  for (i = 0; i < nmed; i++) {
    fprintf(arquivo, "Medidor %d %s %d (%d)\t%d (%d):\t%s\t%d", i,
            charMedidor(medidas[i].tipo), medidas[i].DE, medidas[i].k,
            medidas[i].PARA, medidas[i].m, charFases(medidas[i].fases),
            medidas[i].ligado);
...
void salvaMedidasRedeEletrica(DMED *medidas, long int **numeroMedidas) {
  ...
  fprintf(arquivo,"LISTA DE BARRAS:\n");
  for(i=0;i<numeroBarras;i++){
      fprintf(arquivo,"Barra:
  %d\t%d\t%s\t%.2lf",barras[i].i,barras[i].ID,charFases(barras[i].fases),barras[i].Vbase);
      if(barras[i].nloads>0){
          for(j=0;j<barras[i].nloads;j++)
              fprintf(arquivo,"\n\tCarga
  %d:\t%s\t%.5lf\t%.5lf\t%.5lf\t%.5lf\t%.5lf\t%.5lf\t%.1lf",j,charLigacao(barras[i].loads[j].lig),barras[i].loads[j].Pnom[0],barras[i].loads[j].Pnom[1],barras[i].loads[j].Pnom[2],barras[i].loads[j].Qnom[0],barras[i].loads[j].Qnom[1],barras[i].loads[j].Qnom[2],barras[i].loads[j].ZIP);
      }
      if(barras[i].nshunts>0){
          for(j=0;j<barras[i].nshunts;j++){
              fprintf(arquivo,"\n\tShunt
  %d:\t%s\t%.5lf\t%.5lf\t%.5lf\t%d",j,charLigacao(barras[i].shunts[j].lig),barras[i].shunts[j].Qnom[0],barras[i].shunts[j].Qnom[1],barras[i].shunts[j].Qnom[2],barras[i].shunts[j].controle);
              if(barras[i].shunts[j].controle != 0)
      ...
      if(barras[i].ngds>0){
          for(j=0;j<barras[i].ngds;j++){
              fprintf(arquivo,"\n\tGD %d:\t%s\t%.5lf
  kVA\t%.5lf\t%.5lf\t%.5lf\t%.5lf\t%.5lf\t%.5lf\t%.1lf",j,charLigacao(barras[i].gds[j].lig),barras[i].gds[j].Snominal,barras[i].gds[j].Pnom[0],barras[i].gds[j].Pnom[1],barras[i].gds[j].Pnom[2],barras[i].gds[j].Qnom[0],barras[i].gds[j].Qnom[1],barras[i].gds[j].Qnom[2],barras[i].gds[j].controle);
              if(barras[i].gds[j].controle != 0)

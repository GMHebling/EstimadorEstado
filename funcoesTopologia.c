...
void buscaProfundidade(FILABARRAS *barraAtual, long int idNo, int profundidade,
                       BOOL *visitado, GRAFO *grafo, long int idAlim) {
  ...
  for (i = 0; i < no->numeroAdjacentes; i++) {
    ...
    if ((visitado[barraAdj] == false)) {
      ...
      buscaProfundidade(barraAtual, idNo, profundidade, visitado, grafo,
                        idAlim);
    }
  }
}

void buscaLargura(GRAFO *grafo, ALIMENTADOR *alimentador, long int idAlim,
                  long int idNoRaiz, BOOL *visitado) {
...
void buscaProfundidadeAlimentadores(GRAFO *grafo, long int numeroBarras,
  ...
  if (((*alimentadores) = (ALIMENTADOR *)malloc(numeroAlimentadores *
                                                sizeof(ALIMENTADOR))) == NULL) {
  ...
  for (i = 0; i < numeroAlimentadores; i++) {
    // buscaLargura(grafo, (*alimentadores), i, (*alimentadores)[i].noRaiz,
    // visitado);
    barraAtual = &(*alimentadores)[i].rnp[0];
    buscaProfundidade(barraAtual, (*alimentadores)[i].noRaiz, 0, visitado,
                      grafo, i);
    // printf("\n alimentador %d Raiz: %d   Nos:
    // %d",i,(*alimentadores)[i].idRaiz,(*alimentadores)[i].numeroNos);
  }
}
...
// Converte a matriz de impedância em admitância e monta quadripolo dos ramos do
// grafo
...
void montaQuadripoloTrafo(DRAM *ramo, DTRF *trafo) {
  ...
  if ((trafo->lig_pri == 1) && (trafo->lig_sec == 1)) { // Ligação YN-YN
    ...
    ramo->Ypp[2][0] =
        ramo->Ypp[2][0] + 100 * cabs(y); // Sequência zero para manter o
                                         // quadripólo com posto completo
    ...
    c_matIgual(ramo->Yss, Yii, 3);
    ramo->Yss[2][0] =
        ramo->Yss[2][0] + 100 * cabs(y); // Sequência zero para manter o
                                         // quadripólo com posto completo
    ...
    ramo->Yss[2][0] =
        ramo->Yss[2][0] + 100 * cabs(y); // Sequência zero para manter o
                                         // quadripólo com posto completo
    ramo->Yss[2][1] = ramo->Yss[2][1] +
                      100 * cabs(y); // Pensar se trata-se de simplificação,
                                     // pois pode existir tensão de neutro neste
                                     // caso, mas não irá te corrente de neutro.
    ramo->Yss[2][2] = ramo->Yss[2][2] + 100 * cabs(y);
    ...
    ramo->Ypp[2][2] = ramo->Ypp[2][2] + 100 * cabs(y);
    ramo->Yps[2][0] =
        ramo->Yps[2][0] + 100 * cabs(y); // Sequência zero para manter o
                                         // quadripólo com posto completo
    ...
    // Aproximação para o caso do Y não aterrado - tensão de sequência é igual a
    // zero no secundário - fica igual DYn
    ramo->Yps[2][0] =
        ramo->Yps[2][0] + 100 * cabs(y); // Sequência zero para manter o
                                         // quadripólo com posto completo
    ramo->Yps[2][1] = ramo->Yps[2][1] + 100 * cabs(y);
    ...
    c_matMultEsc(ramo->Ysp, -1, 3);
    c_matIgual(ramo->Yss, Yii, 3);
    //        ramo->Ypp[2][0] = ramo->Ypp[2][0] + 1; //Sequência zero para
    //        manter o quadripólo com posto completo ramo->Ypp[2][1] =
    //        ramo->Ypp[2][1] + 1; ramo->Ypp[2][2] = ramo->Ypp[2][2] + 1;
    //        ramo->Yps[2][0] = ramo->Yps[2][0] + 1; //Sequência zero para
    //        manter o quadripólo com posto completo ramo->Yps[2][1] =
    //        ramo->Yps[2][1] + 1; ramo->Yps[2][2] = ramo->Yps[2][2] + 1;
    ramo->Yss[2][0] =
        ramo->Yss[2][0] + 100 * cabs(y); // Sequência zero para manter o
                                         // quadripólo com posto completo
    ramo->Yss[2][1] = ramo->Yss[2][1] + 100 * cabs(y);
    ramo->Yss[2][2] = ramo->Yss[2][2] + 100 * cabs(y);
    //        ramo->Ysp[2][0] = ramo->Ysp[2][0] + 1; //Sequência zero para
    //        manter o quadripólo com posto completo ramo->Ysp[2][1] =
    //        ramo->Ysp[2][1] + 1; ramo->Ysp[2][2] = ramo->Ysp[2][2] + 1;
  }
  //    printf("\n\Quadripolo\n\n");
  //    c_matImprime(ramo->Ypp,3);
  //    printf("\n");
...
void montaQuadripoloShunt(GRAFO *no, DSHNT *shunt) {
  ...
  ya =
      I * shunt->Qnom[0] / (pow(shunt->Vbase / sqrt(3), 2) / pow(no->Vbase, 2));
  yb =
      I * shunt->Qnom[1] / (pow(shunt->Vbase / sqrt(3), 2) / pow(no->Vbase, 2));
  yc =
      I * shunt->Qnom[2] / (pow(shunt->Vbase / sqrt(3), 2) / pow(no->Vbase, 2));
  ...
  if (shunt->lig == 1) { // Ligação YN
    ...
    Ysh[2][0] = Ysh[2][0] +
                1; // Sequência zero para manter o quadripólo com posto completo
    Ysh[2][1] = Ysh[2][1] + 1;
    Ysh[2][2] = Ysh[2][2] + 1;
...
// Função que transforma os dados do grafo em pu e monta as matrizes de
// admitância dos elementos do grafo -
//  sem busca em profundidade no grafo - Monta Elemento a Elemento de acordo com
//  dados de entrada
void calculaPU(GRAFO *grafo, long int numeroBarras, DRAM *ramos,
               long int numeroRamos, double Sbase) {
  int i, idNo, idRam;
  FILABARRAS *lista_barras = NULL;
  long int barraAdj = 0;
  double Vbase;
  ...
  for (idNo = 0; idNo < numeroBarras; idNo++) {
    ...
    for (i = 0; i < grafo[idNo].barra->nshunts; i++) {
      grafo[idNo].barra->shunts[i].Qnom[0] =
          1000 * grafo[idNo].barra->shunts[i].Qnom[0] / Sbase;
      grafo[idNo].barra->shunts[i].Qnom[1] =
          1000 * grafo[idNo].barra->shunts[i].Qnom[1] / Sbase;
      grafo[idNo].barra->shunts[i].Qnom[2] =
          1000 * grafo[idNo].barra->shunts[i].Qnom[2] / Sbase;
  ...
  for (idRam = 0; idRam < numeroRamos; idRam++) {
    ...
    case 0:
      ramos[idRam].linha.Zaa =
          ramos[idRam].linha.Zaa / ((pow(Vbase, 2)) / Sbase);
      ramos[idRam].linha.Zab =
          ramos[idRam].linha.Zab / ((pow(Vbase, 2)) / Sbase);
      ramos[idRam].linha.Zac =
          ramos[idRam].linha.Zac / ((pow(Vbase, 2)) / Sbase);
      ramos[idRam].linha.Zbb =
          ramos[idRam].linha.Zbb / ((pow(Vbase, 2)) / Sbase);
      ramos[idRam].linha.Zbc =
          ramos[idRam].linha.Zbc / ((pow(Vbase, 2)) / Sbase);
      ramos[idRam].linha.Zcc =
          ramos[idRam].linha.Zcc / ((pow(Vbase, 2)) / Sbase);

      ramos[idRam].linha.Baa =
          ramos[idRam].linha.Baa / ((pow(Vbase, 2)) / Sbase);
      ramos[idRam].linha.Bab =
          ramos[idRam].linha.Bab / ((pow(Vbase, 2)) / Sbase);
      ramos[idRam].linha.Bac =
          ramos[idRam].linha.Bac / ((pow(Vbase, 2)) / Sbase);
      ramos[idRam].linha.Bbb =
          ramos[idRam].linha.Bbb / ((pow(Vbase, 2)) / Sbase);
      ramos[idRam].linha.Bbc =
          ramos[idRam].linha.Bbc / ((pow(Vbase, 2)) / Sbase);
      ramos[idRam].linha.Bcc =
          ramos[idRam].linha.Bcc / ((pow(Vbase, 2)) / Sbase);

      montaQuadripoloLinha(&ramos[idRam], &ramos[idRam].linha);
      break;
    case 1:
      ramos[idRam].trafo.R =
          3 * ramos[idRam].trafo.R / ((pow(Vbase, 2)) / Sbase);
      ramos[idRam].trafo.X =
          3 * ramos[idRam].trafo.X / ((pow(Vbase, 2)) / Sbase);

      montaQuadripoloTrafo(&ramos[idRam], &ramos[idRam].trafo);
      break;
    case 2:
      ramos[idRam].regulador.R =
          3 * ramos[idRam].regulador.R / ((pow(Vbase, 2)) / Sbase);
      ramos[idRam].regulador.X =
          3 * ramos[idRam].regulador.X / ((pow(Vbase, 2)) / Sbase);
...
// Função que transforma os dados do grafo em pu e monta as matrizes de
// admitância dos elementos do grafo
void buscaPU(long int idNo, double Vbase, BOOL *visitado, GRAFO *grafo,
             double Sbase) {
  // Depth-Search Algorithm - busca no e a sua profundidade (gera RNP))
  long int barraAdj, i = 0;
  ...
  for (i = 0; i < no->numeroAdjacentes; i++) {
    ...
    if ((visitado[barraAdj] == false)) {
      ...
      case 0:
        no->adjacentes[i].ramo->linha.Zaa =
            no->adjacentes[i].ramo->linha.Zaa / ((pow(Vbase, 2)) / Sbase);
        no->adjacentes[i].ramo->linha.Zab =
            no->adjacentes[i].ramo->linha.Zab / ((pow(Vbase, 2)) / Sbase);
        no->adjacentes[i].ramo->linha.Zac =
            no->adjacentes[i].ramo->linha.Zac / ((pow(Vbase, 2)) / Sbase);
        no->adjacentes[i].ramo->linha.Zbb =
            no->adjacentes[i].ramo->linha.Zbb / ((pow(Vbase, 2)) / Sbase);
        no->adjacentes[i].ramo->linha.Zbc =
            no->adjacentes[i].ramo->linha.Zbc / ((pow(Vbase, 2)) / Sbase);
        no->adjacentes[i].ramo->linha.Zcc =
            no->adjacentes[i].ramo->linha.Zcc / ((pow(Vbase, 2)) / Sbase);

        no->adjacentes[i].ramo->linha.Baa =
            no->adjacentes[i].ramo->linha.Baa * ((pow(Vbase, 2)) / Sbase);
        no->adjacentes[i].ramo->linha.Bab =
            no->adjacentes[i].ramo->linha.Bab * ((pow(Vbase, 2)) / Sbase);
        no->adjacentes[i].ramo->linha.Bac =
            no->adjacentes[i].ramo->linha.Bac * ((pow(Vbase, 2)) / Sbase);
        no->adjacentes[i].ramo->linha.Bbb =
            no->adjacentes[i].ramo->linha.Bbb * ((pow(Vbase, 2)) / Sbase);
        no->adjacentes[i].ramo->linha.Bbc =
            no->adjacentes[i].ramo->linha.Bbc * ((pow(Vbase, 2)) / Sbase);
        no->adjacentes[i].ramo->linha.Bcc =
            no->adjacentes[i].ramo->linha.Bcc * ((pow(Vbase, 2)) / Sbase);

        montaQuadripoloLinha(no->adjacentes[i].ramo,
                             &no->adjacentes[i].ramo->linha);
        break;
      case 1:
        no->adjacentes[i].ramo->trafo.R =
            3 * no->adjacentes[i].ramo->trafo.R / ((pow(Vbase, 2)) / Sbase);
        no->adjacentes[i].ramo->trafo.X =
            3 * no->adjacentes[i].ramo->trafo.X / ((pow(Vbase, 2)) / Sbase);
        montaQuadripoloTrafo(no->adjacentes[i].ramo,
                             &no->adjacentes[i].ramo->trafo);
        break;
      case 2:
        no->adjacentes[i].ramo->regulador.R =
            3 * no->adjacentes[i].ramo->regulador.R / ((pow(Vbase, 2)) / Sbase);
        no->adjacentes[i].ramo->regulador.X =
            3 * no->adjacentes[i].ramo->regulador.X / ((pow(Vbase, 2)) / Sbase);
        montaQuadripoloRegulador(no->adjacentes[i].ramo,
                                 &no->adjacentes[i].ramo->regulador);
        break;
      }
...
void tranformaPU(GRAFO *grafo, long int numeroBarras, double Sbase,
  ...
  for (i = 0; i < numeroAlimentadores; i++) {
    buscaPU(alimentadores[i].noRaiz, grafo[alimentadores[i].noRaiz].Vbase,
            visitado, grafo, Sbase);
  }
  ...
  /*for(i=0; i<numeroAlimentadores; i++)
  {
      ...
      {
          printf("\nBarra[%d]: %d  Prof: %d  Vbase =
  %.5lf",barraAtual->idNo,grafo[barraAtual->idNo].barra->ID,barraAtual->profundidade,grafo[barraAtual->idNo].Vbase);
...
void atualizaTaps(DRAM *ramos, long int numeroRamos) {
  ...
  for (i = 0; i < numeroRamos; i++) {
    if (ramos[i].tipo == 2) {
      ramos[i].tap_pri[0] =
          (1 + ramos[i].regulador.tap[0] * ramos[i].regulador.regulacao /
                   ramos[i].regulador.ntaps);
      ramos[i].tap_pri[1] =
          (1 + ramos[i].regulador.tap[1] * ramos[i].regulador.regulacao /
                   ramos[i].regulador.ntaps);
      ramos[i].tap_pri[2] =
          (1 + ramos[i].regulador.tap[2] * ramos[i].regulador.regulacao /
                   ramos[i].regulador.ntaps);
      ...
      ramos[i].Yps[0][0] =
          ramos[i].tap_sec[0] * ramos[i].tap_pri[0] * ramos[i].Yps[0][0];
      ramos[i].Yps[1][1] =
          ramos[i].tap_sec[1] * ramos[i].tap_pri[1] * ramos[i].Yps[1][1];
      ramos[i].Yps[2][2] =
          ramos[i].tap_sec[2] * ramos[i].tap_pri[2] * ramos[i].Yps[2][2];

      ramos[i].Ysp[0][0] =
          ramos[i].tap_sec[0] * ramos[i].tap_pri[0] * ramos[i].Ysp[0][0];
      ramos[i].Ysp[1][1] =
          ramos[i].tap_sec[1] * ramos[i].tap_pri[1] * ramos[i].Ysp[1][1];
      ramos[i].Ysp[2][2] =
          ramos[i].tap_sec[2] * ramos[i].tap_pri[2] * ramos[i].Ysp[2][2];
    ...
    } else if (ramos[i].tipo ==
               1) { // REVISAR PARA TAP FORA DA NOMINAL EM TRAFOS
      ramos[i].tap_pri[0] = 1;
      ramos[i].tap_pri[1] = 1;
      ramos[i].tap_pri[2] = 1;
